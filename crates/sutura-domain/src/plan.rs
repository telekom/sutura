...
impl QueryPlan {
    ...
    pub fn resolve_tables<E>(mut self, mut resolve: impl FnMut(&QualifiedTable) -> Result<QualifiedTable, E>) -> Result<Self, E> {
        self.table = resolve(&self.table)?;
        for join in &mut self.joins {
            join.table = resolve(&join.table)?;
        }
        Ok(self)
    }
...
#[cfg(test)]
mod resolve_tables_tests {
    use super::{
        PlanBindings, PlanBucket, PlanColumn, PlanFilter, PlanJoin, PlanKey, PlanMeasure, PlanPredicate, PlanTerm,
        PredicateOrigin, QueryPlan, ResultLabel, StatementTables,
    };
    use crate::calendar::{Date, TimeRange};
    use crate::model::{
        Aggregate, ColumnName, Grain, JoinType, MetricName, QualifiedTable, RelationshipName, SourceName, TableName,
    };
    use crate::warehouse::ParamValue;
