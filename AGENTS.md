...
| `THIRD_PARTY_NOTICES`, the attribution document | the lock file, via `check-attribution` | Generated by `cargo xtask check-attribution --fix`, checked unchanged by the same gate, and shipped - and signed - with every release. The SBOM's crate graph is precise per binary; this list deliberately errs toward the whole resolved set, because attribution errs inclusive. `docs/adr/0021`'s second amendment is the decision |
