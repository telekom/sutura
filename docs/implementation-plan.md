...
| 22 | ~~`build/supply-chain`~~ | nothing - orthogonal | **DONE** - [0021](adr/0021-how-a-published-artefact-proves-where-it-came-from.md). A Sigstore bundle per asset, `cosign` on all six image references, CycloneDX and SPDX per leaf image from the auditable binary, SLSA provenance, ORT in `licence-review.yml`, and REUSE. **The licence half is now built too:** `THIRD_PARTY_NOTICES` is committed and gated by `cargo xtask check-attribution`, released and signed with every tag, and documented in `docs/verifying-a-release.md` - the roadmap bullet's *signed licence reports* is no longer three-quarters |
