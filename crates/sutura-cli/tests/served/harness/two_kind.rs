...
//! needing it.** `daily_usage` used to be the one model this catalog declared with no `via`
//! relationship at all; the compound join gave it one (`daily_usage_subscription`, reached by
//! `data_per_subscription`'s `contract_term` dimension), and moving `daily_usage` alone now makes
//! that relationship's first hop cross sources with a compound key -
//! `without_the_compound_dimension` strips the one dimension that reaches it, the same move
//! `without_the_chained_dimension` makes for the two-source fixture's own chain. Neither of this
//! fixture's own two questions (`recurring_revenue_june`, already in this module's parent;
//! `voice_minutes_by_day`, `two_kind.rs`'s own) ever asked for the dropped dimension, so nothing
//! here loses coverage: each still reads exactly one of the two sources.

use super::postgres::{FixtureLoadGuard, load_into_tier, source_entry};
use super::two_source::without_the_compound_dimension;
...
pub(crate) fn settings(case: &str) -> Option<(String, FixtureLoadGuard)> {
    ...
    without_the_compound_dimension(&catalog);
