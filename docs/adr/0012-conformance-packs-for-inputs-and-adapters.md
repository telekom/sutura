...
- **Orphaned snapshots.** The reference project runs its snapshot tool with unused-snapshot warnings
  ...
  unreferenced check, and AGENTS.md records that it is currently unavailable in the container. With a
  corpus multiplied by adapters that becomes a real gap rather than a nicety, so closing it belongs
  with this work.
...
- `crates/sutura-cli/tests/federation.rs` duplicated about ninety lines of harness deliberately at two
  corpora, with a note that at three the shared module should be extracted. This is the third, so the
  extraction happens here rather than being deferred again.
