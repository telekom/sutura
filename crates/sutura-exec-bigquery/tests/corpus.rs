...
// comparison both live legs are judged by. Shared with `tests/two_datasets.rs`, which is the whole
// reason it is a module: two legs asking the same questions over two dataset layouts have to decide
// agreement the same way, and a second copy of that decision is a second notion of *correct*. Every
// `#[test]` over it stays here, for the reason the naming module gives.
#[cfg(test)]
mod differential;
...
mod tests {
    ...
    use crate::differential::{
        ...
        engine, load_the_corpus, loader, posture_of_the_engine_presented, questions, read_question, source, stem,
    };
    ...
    fn every_corpus_statement_the_compiler_produces_is_accepted_by_the_endpoint() {
        ...
        // One dataset for all four fixtures, which is what this leg is: the closure ignores the
        // model. `tests/two_datasets.rs` passes one that picks by model, over the same loader.
        let loading = loader();
        let loaded = load_the_corpus(&committed, &pinned, &|_model| &loading);
        assert!(
            loaded > 1000,
