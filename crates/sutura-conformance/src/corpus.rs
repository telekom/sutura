...
mod tests {
    ...
    #[test]
    fn the_corpus_holds_the_scalar_boundaries_it_exists_to_compare() {
        let cases = cases();
        let grouped = cases
            .iter()
            .find(|case| case.name() == "total-by-region-and-day")
            .expect("the null-key case exists");
        let region = grouped
            .expected()
            .columns()
            .iter()
            .position(|label| label == "region")
            .expect("the null occupies the region group key");
        assert!(
            grouped
                .expected()
                .rows()
                .iter()
                .any(|row| matches!(row.get(region), Some(Value::Null)))
        );

        let mean = cases
            .iter()
            .find(|case| case.name() == "mean-by-day")
            .expect("the approximate case exists");
        let mean_measure = mean
            .expected()
            .columns()
            .iter()
            .position(|label| label == "amount_mean")
            .expect("the mean is the aggregate column");
        assert!(
            mean.expected()
                .rows()
                .iter()
                .all(|row| matches!(row.get(mean_measure), Some(Value::Real(_))))
        );

        let wide = cases
            .iter()
            .find(|case| case.name() == "wide-total-by-day")
            .expect("the wide case exists");
        let wide_measure = wide
            .expected()
            .columns()
            .iter()
        ...
        assert!(wide.expected().rows().iter().any(|row| {
            matches!(
                row.get(wide_measure),
                Some(Value::Text(value)) if value.parse::<i128>().is_ok_and(|value| value > i128::from(i64::MAX))
            )
        }));
        assert!(
            wide.expected()
                .rows()
                .iter()
                .any(|row| matches!(row.get(wide_measure), Some(Value::Integer(_))))
        );

        let overflow = cases
            .iter()
            .find(|case| case.name() == "overflowing-integer-total-by-day")
            .expect("the integer-sum boundary case exists");
        let overflow_measure = overflow
            .expected()
            .columns()
            .iter()
        ...
        assert!(
            overflow
                ...
                .rows()
                .iter()
                .any(|row| { matches!(row.get(overflow_measure), Some(Value::Text(value)) if value == "9223372036854775808") })
        );
        assert!(
            overflow
                .expected()
                .rows()
                .iter()
                .any(|row| matches!(row.get(overflow_measure), Some(Value::Integer(6))))
        );
        ...
        assert!(
            decimal
                ...
                .rows()
                .iter()
                .all(|row| { matches!(row.get(decimal_measure), Some(Value::Text(value)) if value.contains('.')) })
        );
    }
