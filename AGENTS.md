...
**The third case in this section is a whole ADAPTER, and its missing piece is now narrower still -
narrow enough that naming it precisely is the whole value of this paragraph.**
`sutura-exec-bigquery` implements `Warehouse` completely - the credential match, the posture
agreement, the rendering, the leg refusal and the value mapping, all built and all tested - and since
...
second narrow port whose one implementor reads the file `just gcloud-login` writes.
...
**What has still never happened:** no composition root links the crate, `sutura-serve` refuses
`kind: bigquery` by name, and the `data_systems:` axis of the golden matrix still gains no entry - one
live statement is not a registered data system. The DIALECT axis does have one, and everything in
*Invariants* about rendering, quoting, binding and the row cap holds for four dialects rather than
three.
