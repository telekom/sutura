...
| Third-party attribution | `Cargo.lock` plus `cargo metadata` | `ATTRIBUTION.md` is GENERATED - `just attribution` writes one row per third-party crate with the SPDX expression its manifest declares, and `cargo xtask check-attribution` fails when its crate set drifts from the lock. **Deliberately WIDER than any one binary**, which is the opposite of the SBOM's rule and for a stated reason: an SBOM overstating what is in an artefact is a false claim about it, while an attribution document naming a crate that did not ship discharges an obligation nobody had - so the two err in opposite directions on purpose. `docs/adr/0021`'s attribution amendment is the decision. **The gate reads the lock and the document and invokes nothing**, which is what lets it be a hygiene gate; the licence COLUMN is therefore not gated, only the crate set. **Two limits**: neither this nor the ORT report carries the notice text an Apache-2.0 dependency's own `NOTICE` file holds - that needs a source scan, deliberately absent - and the two vendored path crates have no registry source, so `VENDOR.md` and `REUSE.toml` attribute them instead |
