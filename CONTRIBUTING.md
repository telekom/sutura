...
you split: the gate then verifies the commit moved test code and changed none - every changed path
compiled Rust, every added and removed line test code in its own image, the trimmed code-line
multiset equal on both sides - and answers `nothing to measure`. **The trailer is a claim, not a
permission**: one added, deleted or reworded line and the gate fails and names it. `sutura/gates`
carries the four conditions and what that pass does not prove.
