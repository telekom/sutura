...
| A bundle naming an absent table does not boot | `Warehouse::preflight`, one call per dataset | A dataset that could not be LISTED is a `WARN`, not a refusal - the alternative being a skip flag set in the deployment that most needs it. No live dataset has answered one |
