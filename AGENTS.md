...
| No leg of an ANSWER reaches a data system without a credential somebody minted for it | `Warehouse::execute` takes a `&Presented` and has **no default**, so there is no signature that runs as whatever the process is - a `compile_fail` doctest with a compiling twin, differing by that one argument. `dry_run` takes it too and returns `PreFlight`, whose default is `NotAsked`, so an adapter that did not ask cannot be read as having accepted. The credential comes from `identity::CredentialBroker::mint`, called **once per answer for every source the plan reads**, and `LegCredentials` hoists the asker and the deadline out of the legs: one `asked_by`, one `not_after`, a private `by_source` with no `insert`, and a constructor that refuses a set not covering the sources it was minted for. `Presented` has three variants and the third carries no credential material at all, so the shared posture is a shape a reader sees rather than a value an adapter ignores. **And the leg is compared against the declared posture in `sutura_app::answer`**, through `Presented::agrees_with` - one exhaustive match over the pair - so a shared leg carrying a *different* operator acknowledgement, or a subject shape against a source declared shared, is `ServiceError::Posture` instead of a leg that executes and is then reported under the adapter's declaration. The two values are independent by construction: a broker reads the settings tree and the registry holds what the composition root opened. **It is in the application and not only in each adapter, and a review moved it there:** both shipped adapters do make the comparison, `Warehouse` is a trait, and this workspace's own fake omitted it - so an `impersonation-at-source` fake handed the deployment's own identity executed and provenance reported it as impersonated. Made once in `answer`, the rule reaches every adapter a registry can hold; the adapters keep their own copy because an adapter is the last thing before a driver. **The limit, because the witness is prose:** equality is the comparison available, so a fabricated witness whose text is byte-for-byte this source's is indistinguishable from it. **And what the broker answered is compared with the request it was made for, once, before anything reaches an adapter.** `Minted::agreeing_with` is that one guard: the grant's subject must equal the subject the transport established, the grant must cover exactly the sources the plan reads, its deadline must not have passed, and a refusal must name a source that was asked about. Any disagreement is a typed `Err` and never a refusal - a broker contradicting the request says nothing about the caller's access, and offering it as `CredentialUnavailable` would both mislead the asker and invite a retry loop over a wiring defect. **It is not skippable by placement**, which is the difference between this and the three checks it replaced: `BoundToTheRequest` is the only type that hands out a `Presented`, its field is private, and `agreeing_with` is its only constructor - so the path from a broker's answer to a value an adapter executes with runs through the comparison. Added by a review that reproduced all three holes with a diagnostic broker: a grant minted for another subject was executed and answered, `Expiry` was computed and read by nobody, and a refusal naming a source nobody asked about reached the caller as a `403` about that source. **Three limits, and the first is the one to read:** no adapter in this build can carry a per-subject credential - both declare `NoPlaceForASubject` and return a typed `Err` for the two subject shapes - so what a broker can mint here is the deployment's own identity, and *no source executes as the asking subject yet*. The deadline guard runs ONCE, so a credential whose life is shorter than a pre-flight plus an execution can still age out at the data system, which is where it would be refused - the re-check needs a clock the application can advance in a test rather than a `SystemTime` call, and `docs/adr/0008` part 6 still puts the *floor* - enough life left for what this query may take - in the broker adapter. And the port takes the `RequestContext` and **not** the caller's own assertion, because `docs/adr/0014` Decision 3's exchange is blocked on a verification nobody has done - a `Secret` field now would be the guess this port was delayed to avoid. **The row says ANSWER for a reason:** the BOOT path reaches a data system through `Warehouse::verify_anchor`, which takes no credential and runs under whatever identity the adapter was configured with - see the unvalidated-bundle row |
...
- **Least authority on the execution leg.** End-to-end impersonation is the point of the product: a
  ...
  with no credential is the boot path's, and a review is what made that sentence safe to write:**
  `Warehouse::verify_anchor` takes an `AnchorPlan`, which parses a plan as a declared anchor's own -
  refusing a grouped plan, a requested predicate, another metric and another range - so the path with
  no credential cannot be handed a question. Its constructor is `pub`, so that narrows the door rather
  than closing it, and the type says so. What is NOT built is a
