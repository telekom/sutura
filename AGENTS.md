...
| `sutura-exec-bigquery` | `Warehouse` over BigQuery: renders through `sutura-sql` in the fourth dialect and pushes down. **Everything it DECIDES is built and tested against a fake; the WIRE is not** - `transport::JobTransport` has no implementor that speaks to the endpoint, no composition root links it, and `docs/adr/0017` says why the change that adds one is the change that can first verify it. In *Built And Not Wired*, not in *Invariants* |
| `sutura-exec-postgres` | `Warehouse` over Postgres as a data source: renders through `sutura-sql` and pushes down over the wire. A dev-dependency, not shipped. Its driver is pure Rust, which is the artifact question answered in code. Demonstrates the static-credential half only; row 18 (impersonation) changes it. **Its corpus cells run ONLY where a tier is provisioned** - a dev machine after `just dev-up`, or the `tier-tests` CI job that provisions a service container - and skip loudly in any sandbox with none; the one source that proves acceptance over the wire. `NoTls`, localhost-tier-only (the connection is unencrypted, whatever SCRAM protects) |
...
**The third case in this section is a whole ADAPTER, and its missing piece is narrower than the other
two.** `sutura-exec-bigquery` implements `Warehouse` completely: the credential match, the posture
agreement, the rendering, the leg refusal and the value mapping are all built and all tested. What is
absent is a `transport::JobTransport` that speaks to the endpoint - so the crate has never sent a
statement anywhere, and `sutura-serve` links it not at all and refuses `kind: bigquery` by name.
`docs/adr/0017` decides that deliberately rather than incidentally: **nothing in this repository can
verify a network client**, so the change that writes one is the change that can first run it against a
real project, and that is also where the HTTP-and-credential dependency decision belongs. The
`data_systems:` axis of the golden matrix therefore gains no entry, because that registry's own rule is
that a cell which cannot execute reads as coverage. The DIALECT axis does gain one, and everything in
*Invariants* about rendering, quoting, binding and the row cap now holds for four dialects rather than
three.
