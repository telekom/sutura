...
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sutura-exec-datafusion` (shipped)      | `tokio::time::timeout(remaining, rows)` inside `block_on`, on a runtime that now enables its timer; dropping the future drops the stream, and the engine's own spawned partition tasks request abort on drop (`datafusion-common-runtime`'s `SpawnedTask`). The physical optimizer's default `EnsureCooperative` rule wraps a non-cooperative leaf or eager boundary that has no cooperative ancestor, so the plan has yield points where a dropped future can land                                                                                                                                                                                                                                                                                               | The engine is this process, so *the data system* is the engine's operators: work stops at the next yield point, memory reservations are released with the operators                                                                                               | Planning (analyzer, optimizer) is synchronous inside the future and is not interrupted; one started blocking file read (one range of one file, `object_store`'s `maybe_spawn_blocking`) runs to its end; nothing after it is polled                                                                                                                                                                                                                                                                                    |
...
| The engine returns after the deadline at a cooperative yield | `sutura-exec-datafusion` wraps `rows` in `tokio::time::timeout` on a runtime built with `enable_time()`. Two runtime tests register a `StreamingTable` that does not finish on its own, open a 200ms budget, and assert the call returns inside the budget plus a 300ms tolerance and before the source's own 2s end; the predicate answers `true` and the source's `Drop` fires. A third test proves a spent deadline never lets the plan touch the source by counting executions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | A non-yielding operator, planning (the analyzer and optimizer), and one started blocking file read stay unreached. **The stop is at the next cooperative yield, not at the instant**: each runnable partition task spends a 128-batch budget before the timer is heard, so overrun scales with `target_partitions` and batch cost; measured 825ms against a 200ms budget with 12 partitions on the current-thread runtime and 290ms with 2 workers. Dropping the rows future requests cancellation of spawned asynchronous tasks; already-running blocking work is not synchronously killed and may outlive the refusal                                                                                                                                                                                                                                                                     |
...
- **On the engine's current-thread runtime, a stopped question's task tree is reaped on the NEXT
  `block_on`, not when `execute` returns.** Aborting a partition task is a request the runtime
  services at that task's own next poll (see the port's own limit above), and a current-thread
  runtime has no turn to give it without a caller entering `block_on` again - measured with a
  spinning source: 312 batches produced by the time `execute` returned, 325 after one further
