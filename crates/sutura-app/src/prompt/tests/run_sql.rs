...
#[test]
fn a_deployment_that_turns_run_sql_on_does_not_contradict_its_own_no_field_section() {
    // Before this module existed, "What this surface has no field for" said "do not ask a human to
    // enable it for you" unconditionally - which reads as false the moment a human did. A document
    // that both says that and lists `run_sql` under "The operations you have" is internally
    // inconsistent, which is worse than either sentence alone.
    ...
    assert!(
        ...
        "the caveat pointing at the exception is missing:\n{text}"
    );
}
