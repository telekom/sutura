...
| The measure vocabulary is closed, and holds no SQL expression | `Measure` is an enum of three shapes and `RequiredFilter` an enum of four operators, both with `deny_unknown_fields`. There is no field a catalog could write `sum(price * quantity)` into, and an unrecognised shape is an error naming what it found rather than a key silently dropped. A fourth shape is a domain variant plus a plan variant plus a generator arm per dialect plus a golden - a code change by design, not a catalog edit. `docs/adr/0002-a-closed-vocabulary-for-measures.md` says why the closed set is the mechanism and "one aggregate" was only ever a means to it |
