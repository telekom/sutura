...
Status: **accepted; the record, port signature and refusal have landed. The engine now returns at a
cooperative yield after its deadline and drops the rows future; Postgres stops its certified path
with `statement_timeout`. BigQuery still accepts and ignores the port deadline. Postgres's raw SQL
path has no per-request deadline and remains bounded by its connect-time ceiling.** Four accepted
