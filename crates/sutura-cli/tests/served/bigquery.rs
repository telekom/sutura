...
mod tests {
    ...
    use crate::harness::{
        ...
        without_the_compound_dimension, written,
    };
    ...
    ///
    /// `daily_usage.md` for `harness::two_kind`'s recorded reason: it used to be the one model the
    /// example catalog declared with no `via` relationship at all; the compound join gave it one,
    /// which is exactly why `without_the_compound_dimension` runs on every catalog this file
    /// derives - see that function's own note.
