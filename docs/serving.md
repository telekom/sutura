...
`SET LOCAL statement_timeout` after acquiring its execution lock. **BigQuery sends the deadline
nowhere.** `timeoutMs`/`jobTimeoutMs` were `jobs.query` request parameters on the HTTP transport,
which is deleted; the ADBC driver is given no bound by this process, so the caller's wait is bounded
and the job behind it is not - nothing cancels a running BigQuery job. The concurrency bound is
therefore load-bearing rather than belt-and-braces here: it caps how much work can accumulate where
cancellation is delayed, cooperative, or - on this one source - absent.
