...
socket - a mismatched address cannot masquerade as already-up. A `just dev-endpoint postgres`
between the `dev-up` and the next `just test` still answers nothing.
