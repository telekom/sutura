...
| `qb4o:ManyToMany` as a `JoinType`                 | The domain has three cardinalities and QB4OLAP has four. Folding the fourth onto `OneToMany` manufactures the warrant `Definitions::assemble` refuses a dimension through as `JoinWouldDuplicateRows`, so that fold is a **refusal** rather than a wrong answer - `JoinType::may_duplicate_rows` is `matches!(self, Self::OneToMany)`. Folding it onto `ManyToOne` or `OneToOne` is the fold that answers: no refusal fires, and the result is a wrong number with a provenance record that looks clean. Neither fold is available, which is why the cardinality has to be refused at the source. A cardinality the source states and this domain cannot hold is ADR 0002's decision, not a connector's. |
