...
when somebody asks for it, which is the general rule `AGENTS.md` states for an adapter with a native
or an outbound-TLS dependency, and it is the same shape this record's SECOND amendment already took
for `sutura-serve`.
