...
//!   see [`corpus::variants`] for the two ways the filesystem disagreed with it. What that costs, stated:
...
//! decode as UTF-8, a cell whose run this cannot decide, and a variant nothing reaches. More refuse
//! before it, in [`run`]: no repo root, no crate directory for this gate itself, an unreadable root
//! manifest, a member list this cannot state, and a corpus file [`reach::evidence`]'s own loop
//! skipped. What used to be a SIXTH - the corpus walk itself leaving a path with no verdict - is now
//! [`repo::Refusal`], returned by [`repo::all_files`] and by [`corpus::Corpus::read`] rather than
//! reconstructed as this file's own arithmetic.
//!
...
//!
//! **One of the arms is a pair rather than a number**, and `github.com/telekom/sutura#414` is why:
...
//! read, in [`reach::evidence`] - which does no IO and so cannot hand its walk to
//! [`repo::Census::inspect`], leaving `evidence`'s own `looked != files.len()` check as a SOURCE
//! mutation no input tree can express, held by mutation review rather than by a fixture.
//! [`corpus::Corpus::read`]'s equivalent check is gone rather than paired: since this gate migrated
//! onto `Census::inspect`, `judged + out_of_scope + absent == discovered` holds by construction,
//! so a truncated corpus walk is now a [`repo::Refusal`] rather than a claim this file's own
//! arithmetic had to defend.
...
use corpus::{Corpus, EXAMPLES, Members, gate_crate};
use reach::{Evidence, evidence, report};
...
pub(crate) fn run(_args: &[String]) -> Verdict {
    ...
    let census = match repo::all_files() {
        Ok(census) => census,
        Err(why) => {
            eprintln!("xtask check-examples: {}", why.describe());
            return Verdict::Fail;
        }
    };
    let scan = match Corpus::read(census, &members, crate_dir) {
        Ok(scan) => scan,
        Err(why) => {
            eprintln!("xtask check-examples: {}", why.describe());
            return Verdict::Fail;
    ...
    let evidence = match evidence(scan.sources(), scan.published()) {
    ...
    let problems = problems(scan.variants(), &evidence, &scan);
