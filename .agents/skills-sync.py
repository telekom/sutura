...
def fetch(repo: str, ref: str, into: Path) -> bool:
    ...
    try:
        subprocess.run(
            ['git', 'clone', '--quiet', '--filter=blob:none', '--no-checkout', repo, str(into)],
            check=True, capture_output=True, text=True,
        )
        subprocess.run(['git', '-C', str(into), 'fetch', '--quiet', '--depth', '1', 'origin', ref],
                       check=True, capture_output=True, text=True)
        ...
        print(f'    fetch failed: {detail.strip().splitlines()[-1] if detail.strip() else exc}')
        return False
...
def refresh_mirror(entry: dict[str, str], clone: Path) -> str:
    ...
    if keep:
        ...
        if lines and lines[0].strip() == '---':
            close = next(i for i, l in enumerate(lines[1:], start=1) if l.strip() == '---')
            text = '\n'.join(lines[: close + 1]) + '\n\n' + keep + '\n'.join(lines[close + 1:]).lstrip('\n')
        else:
            text = keep + text
    with io.open(dst / 'SKILL.md', 'w', encoding='utf-8', newline='\n') as fh:
...
def main(argv: list[str]) -> int:
    ...
    for entry in entries:
        ...
        if not args.offline and entry['status'] == 'generated':
            ...
            out = subprocess.run([sys.executable, str(gen)], capture_output=True, text=True, check=False)
            note = 'generator ok' if out.returncode == 0 else 'generator failed'
            print(f'  {entry["name"]}: {out.stdout.strip().splitlines()[-1] if out.stdout.strip() else note}')
    ...
    payload = {'version': 1, 'skills': sorted(locked, key=lambda s: (s['catalog'], s['name']))}
