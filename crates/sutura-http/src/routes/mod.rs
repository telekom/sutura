...
//!   present, so this is always reachable - which is why its body carries nothing.
//! * [`protected_resource`] is unversioned and unauthenticated too, when direct inbound identity is
//!   configured - the other path that is always reachable. It tells a client which authorization
//!   server governs that exact resource.
