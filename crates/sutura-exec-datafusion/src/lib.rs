...
use crate::translate::{bucket_expression, column, key_uniqueness_plan, measure_expression, predicate, table_reference};
...
impl DataFusionWarehouse {
    ...
    ///
    /// The plan itself is `translate::key_uniqueness_plan`'s own doc comment; this is the
    /// execute-and-read half every other query method here follows.
    async fn key_uniqueness(&self, key: &DeclaredKey<'_>) -> Result<KeyUniqueness, DataFusionError> {
        let scan = self.scan(key.table()).await?;
        let logical = key_uniqueness_plan(scan, key)?;
        let frame = self
            .context
            .execute_logical_plan(logical)
            .await
