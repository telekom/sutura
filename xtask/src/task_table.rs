...
//! `github.com/telekom/sutura#610`. Measured on the base of this move: a BARE registration - a
//! `mod` line plus the seven-line entry - reached exactly 1000 and PASSED with nothing to spare
//! (`lines <= max` is the predicate), and one line more, the *why it is here* comment most entries
//! here carry, gave `max-lines: FAILED - 1 file(s) over 1000 lines`. So the cap refused this
//! table's own convention, which is a WALL rather than a forcing function: it does not push a new
//! rule toward an arm on a gate that already walks the right files - usually the better shape, and
//! named as such in the `sutura/gates` skill - it removes the choice between the two.
//!
//! **The cap did not block a registration, and that correction outranks the issue's count.**
//! `max-lines` passes at `lines <= max`, so from 992 a bare registration landed on exactly 1000
//! and PASSED - the measurement above. Of the two changes `#610` cites as redirected, neither
//! one's own record blames the cap: the guardrails change wrote that its arm *"needs no task
//! registration"* and named the lane holding `TASKS`, and the image-record reader argues from its
//! seam and mentions no cap at all. A third branch did register a task, 992 to 999, and was
//! closed for unrelated reasons. Three files in this tree cited the cap and are not that pair -
//! `workflows/scorecard.rs` and its parent `workflows.rs`, which called it *"partly"* the reason
//! for a rule both also call right regardless, and the `sutura/gates` skill; all three are
//! corrected here. A fourth reached `main` after this branch point, a test module deferring a
//! gate that `check-serde-parse`'s existing walk could carry, and is left to its owner. Every one
//! of them, zero wrong outcomes: the count was never the argument. What is left is structural - the cap refuses this table's own comment convention one
//! registration from here, and the cheapest escape from that refusal is shaving unrelated lines
//! out of `main.rs`, which pays for a structural problem with a diff that looks like tidying.
//!
//! **`registry.rs` recorded the opposite decision, and its reason was circumstantial.** That
//! header says the TYPES moved and *"`TASKS` itself stayed, and that is deliberate rather than
//! arbitrary: the table is where every new gate registers, so moving it would put a rename in the
//! path of every concurrent branch"*. That cost is real and it is paid by branches that register a
//! gate - which the cap was one line from refusing, so the cost it warns about was zero when it
//! came to be measured. Measured before the move: of the open pull requests, NONE touched
//! `xtask/src/main.rs`, and none of the linked checkouts on the machine that made this change held
//! an uncommitted edit to it. The rest of that header - that `main.rs` is held at HEAD by the tests
//! that read it either way - argues the move is SAFE, not that the table should stay: those tests
//! call the fn pointer out of `TASKS` and cannot tell which file declares it.
//!
//! `TASKS` is re-exported from the crate root, so `crate::TASKS` resolves unchanged for `main`'s
//! dispatch, `usage`, `run_hygiene`, `task_names`, `hygiene_gates` and `falsifier`'s sweep.
//!
//! # What this does not hold
//!
//! It does not make the table cheaper to merge: two sessions adding a gate still collide on the
//! same lines, in a different file - `TASKS` is a merge-time contention point wherever it lives.
//! The cap still refuses the file that holds it, at about forty further entries rather than none.
//! And it fixes ONE file: nothing here reports how close any file is to the cap, so the next
//! near-cap file is as invisible as this one was - `#610`'s own stated limit, and the reason no
//! line count is transcribed into prose by this commit.
//!
//! And it costs a registration one more edit SITE than `main.rs` did: the `mod` line there, the
//! entry and an import here. Measured while falsifying this move - a probe entry's `run:` target
//! did not resolve until the module was named in the `use crate::{..}` list below, which the
//! crate root never needed.

use crate::registry::{Falsifier, Kind, Reads, Task};
use crate::run_hygiene;
use crate::{
    action_shell, api_docs, api_links, arrow_major, attribution, boot_order, boundaries, bounded_wait, branches, causality,
    changes, commit_msg, compose, conformance, crap, default_feature_tests, default_features, devenv_linter, devenv_shell, docs,
    examples, feature_remedies, fmt, fuzz, gate_classification, guidance, hook_coverage, hooks, inconclusive, jscpd,
    line_endings, max_lines, newtype_leaks, nix_platform, one_bound, orphan_modules, pins, refusals, release_provenance,
    serde_parse, shared_client, shipped, skills, tasks, text, threshold_expect, unused_deps, venues, warm_start, workflows,
    worktree_state,
};
