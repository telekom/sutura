...
**What has still never happened is a statement reaching a real dataset.** `docs/adr/0017` predicted
that the change writing the wire would be the change that could first run it; it was not - the machine
had no credential and no project - and 0018's *What is still not claimed* section records that rather
than leaving it to be inferred from the code existing. Concretely: no composition root links the crate,
`sutura-serve` refuses `kind: bigquery` by name, the acceptance leg
(`crates/sutura-exec-bigquery/tests/acceptance.rs`, reached by `just bigquery-acceptance`) is
`#[ignore]`d and **unexecuted**, and the `data_systems:` axis of the golden matrix still gains no entry
because that registry's own rule is that a cell which has never executed reads as coverage. The
DIALECT axis does have one, and everything in *Invariants* about rendering, quoting, binding and the
row cap holds for four dialects rather than three.

**The acceptance leg is also the one gap `just validate` structurally cannot close**, and that is
worth knowing before reading a green run as coverage: the nix check sandbox has no network, this
repository is public so a workflow secret is unavailable to a fork's pull request, and a gate that
fails for an environment reason gets disabled. So acceptance evidence for this dialect lives in a
developer's terminal and nowhere else.
...
| A `BigQuery` job can be submitted over HTTP, and what this adapter puts on the wire is the document it says it is | `sutura_exec_bigquery::wire`, behind a **default-off `wire` feature** that every gate still compiles because they all pass `--all-features` and `deny.toml` sets `all-features = true`. `BigQueryWire` is the one `JobTransport`: `jobs.query` over `ureq` (blocking, because the port is; rustls, because two release triples are musl; **zero new packages in `Cargo.lock`** - 446 before, 446 after - because `libduckdb-sys` already resolves that version and feature set). `wire::credential::AccessTokens` is a second narrow port and the seam per-subject execution arrives at; `ApplicationDefault` is its one implementor, reading the file `just gcloud-login` writes and refusing the four other credential shapes BY NAME. Four decisions the suite pins by value: one page or a refusal (`pageToken`, an incomplete job, or a total the delivered count does not equal), the service's own result cache OFF (an anchor reproducing from a cache has reproduced the cache), `max_redirects(0)` so the bearer has no second host, and every foreign string bounded and character-filtered - the endpoint's `reason` kept and its free-text `message` not a field on the error type at all. Nothing is cached, deliberately: `clippy.toml` disallows the primitive, *a credential is not reused past its expiry* is what the per-subject step owes, and a token cache keyed by nothing is the credential-shaped version of the result cache this crate already refuses. Twenty-six tests, asserting the SERIALIZED request body rather than a struct | **An executed run, and it is the whole of what is missing.** `docs/adr/0017` predicted the change writing the wire could first verify it; **it could not** - no `gcloud`, no credential, no project on that machine. So the acceptance leg is `crates/sutura-exec-bigquery/tests/acceptance.rs`, three `#[ignore]`d tests behind `just bigquery-acceptance`, and it is UNEXECUTED. `docs/adr/0018` states the two limits with it: the response documents the suite decodes were written here rather than by the service, and **the HTTP exchange itself is untestable locally by construction** - the host is a `const` and the agent is `https_only`, so there is no loopback to point it at, which is a security property paid for with a coverage hole. And `just validate` cannot close it: the nix sandbox has no network, and a public repository cannot hold a secret a fork's pull request could use |
