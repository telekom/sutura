...
| A caller's identity, where one is established, comes from a signature and never from a header | `sutura_config::InboundIdentity` is a closed enum with no default: `security.inbound` with no `mode` is `SettingsError::InboundModeUndeclared` and the process does not start, because defaulting either way is wrong in opposite directions. Its `BehindGateway` variant carries a `TransitProof` whose fields are an issuer, an audience, a key set and a pinned algorithm - there is **no field for the name of a header holding a username**, and a test presents such a header and gets nobody. Algorithm confusion is unrepresentable in two places rather than checked: `SigningAlgorithm` has no `None` and no `HS*` variant, and `sutura_http::inbound::keys` refuses an `oct` key in a key set. The audience is compared against this deployment's own resource identifier with `aud` in `required_spec_claims`, so a token carrying none is refused rather than passing a check with nothing to compare. **Three limits, each stated where the claim is:** keys come from a file and there is no JWKS endpoint; the two metadata documents `docs/adr/0014` describes are not served; and `Scopes` is parsed, carried and read by nothing. `docs/adr/0014`'s *What is built* section is the authority on all of it |
...
| A forged key id cannot turn every request into an outbound call | `sutura_http::inbound::keys::KeySetCache` refetches on an unknown key id at most once per `MIN_REFETCH_INTERVAL`, measured from the last **attempt** rather than the last success - so a source that is down is limited too. `key_for` takes the instant as a parameter, which is what makes the case that matters - the second forged id inside the window - assertable without a sleep; a test spends ten forged ids for one read. **The limit:** the only source that ships reads a local file, so what the limit protects today is this process rather than an authorization server |
...
- **Least authority on the execution leg.** End-to-end impersonation is the point of the product: a
  ...
  `security.inbound` knows who is asking, from a signature. **Leg 2 is not**: no source executes as
  that person, there is no credential per leg, and a second execution leg has no per-leg credential to
  test against. So a deployment can now name the subject in every audit record and still read every
  row as one identity, which is the confusion `docs/adr/0014` and
  `docs/adr/0010` both warn about - and why the startup log prints the limit beside the mode rather
  than only the mode.
