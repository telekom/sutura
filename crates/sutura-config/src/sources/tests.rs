...
fn a_relative_path_is_refused_at_parse() {
    ...
    assert_eq!(
        registry.get(&alias("local")).expect("the entry is there").data_dir(),
        PathBuf::from(DATA)
    );
}
...
fn a_kind_this_build_has_no_adapter_for_is_refused_at_parse() {
    ...
    let error = SourceRegistry::parse(&entries, Some(&single_user())).expect_err("this build has no BigQuery adapter");
