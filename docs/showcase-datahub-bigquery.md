...
- **`--datahub tier`** (the hosted job's) starts the REAL docker DataHub platform
  ...
  exactly as `datahub-acceptance` does, mints a real personal-access token through the tier's token
  surface (never a committed token - it is written to a generated `token_file` at run time), and
  points the served binary's HTTP `AspectReader` at the platform. Same one cell, same three asks,
  same anchors - fail-not-skip on every step.
