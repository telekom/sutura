...
| `sutura-exec-bigquery` | `Warehouse` over BigQuery: renders through `sutura-sql` in the fourth dialect. The adapter and its decisions are built against a fake; the wire (`transport`) has no implementor, which `docs/adr/0017` decides |
| `sutura-exec-postgres` | `Warehouse` over Postgres on a static credential (the shared identity, no impersonation). Pure-Rust driver, so links nothing. `NoTls`, localhost tier only. A dev-dependency. Its corpus and differential cells are fail-closed: inside `checks.nextest` on the sandbox's own Unix-socket server (`nix/postgres-tier.nix`), or against `just dev-up`'s docker tier |
