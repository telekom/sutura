...
#   `cargo xtask <task>`  names come from the `TASKS` table in xtask/src/task_table.rs, which
...
if [ -z "$(printf '%s' "$gates" | tr -d ' \t\n')" ]; then
    echo "check-task-citations: parsed no task names out of xtask/src/task_table.rs" >&2
    echo "  the scan is broken, not the prose" >&2
    exit 2
fi
...
printf '%s\n' "$recipes" | awk -v gates="$gates" '
    ...
    END {
        if (bad) {
            ...
            print "xtask/src/task_table.rs - not a list in this script." > "/dev/stderr"
            exit 1
        }
