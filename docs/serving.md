...
| `metrics_span_different_models`          | `422`  | The named metrics do not share a model, time column, grain and dimensions. Ask about them separately - one grouped statement needs all of them to agree                                                                                                                                                                                                                                                                                                                                                                                                       |
| `too_many_metrics`                       | `422`  | More metrics than one question may name together. Ask fewer - the sentence carries the maximum                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `duplicate_metric_name`                  | `422`  | The same metric was named twice. Send it once                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `multi_metric_federation_not_executable` | `409`  | Ask about each metric separately. This build does not federate several metrics, and the question reached a dimension on another data system                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `multi_metric_top_not_executable`        | `409`  | Ask about one metric with `top`, or ask about several with no `top`. `top` names no metric to rank by, and this build's two rendering paths disagree about which measure it would mean once more than one is named                                                                                                                                                                                                                                                                                                                                            |
...
first half does not survive checking: nothing mainstream retries a `4xx` by default, and 10 refusal
...
| `catalogs[].kind`                               | `markdown`                           | Which adapter opens this catalog. `markdown`, `datahub`, `okf`, `datacontract`, `openmetadata` or `rdbms`. `datahub` opens behind its default-off `--features datahub`; a build without the feature refuses the kind by name, naming it. `okf` and `datacontract` open on EVERY build (`sutura-catalog-okf` and `sutura-catalog-datacontract` are unconditional dependencies of the binary). `openmetadata`/`rdbms` are declarable and refused by name on every build (no reader over a real deployment yet - see the follow-up to `github.com/telekom/sutura#152` and `#972`). **A deployment declares catalogs of ONE kind** - a `markdown` and a `datahub` entry together are refused, not merged; see `docs/adr/0016` |
| `catalogs[].dir`                                | `catalog`                            | `markdown`/`okf`/`datacontract` only for its meaning (for `datacontract`, a directory of ODCS v3 contract YAML documents); **required non-empty for every kind, including `datahub`**, which does not read it - a stated limit of the settings surface rather than a per-kind field. For `markdown`/`okf`/`datacontract`, at most 1,000 documents / 16 MiB of documents in total; more is a startup refusal naming the bound; raising it is a source change, not a setting                                                                                                                                                                                                                                                |
| `catalogs[].data_dir`                           | `data`                               | `markdown` only for its meaning; **printed by the startup banner and read by nothing that opens a data system** even for a `markdown` catalog - a served source's files come from its own `sources.<alias>.data_dir`, and the `sutura` command reads that same entry or else the directory on its command line. Required non-empty for `datahub`, `okf` and `datacontract` too, unread by any of them, the same stated limit as `dir` above                                                                                                                                                                                                                                                                               |
