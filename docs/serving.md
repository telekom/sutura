...
| `sources.<alias>.host` | absent | Postgres, ClickHouse and Oracle. A DNS name or IP address. On Postgres, exactly one of `host` and `unix_socket`; on ClickHouse and Oracle it is the only dial, and on Oracle it must be a loopback address because `plaintext` is that kind's only mode |
...
| `sources.<alias>.service_name` | absent | Oracle only. Required: the service the listener resolves, the path of an EZCONNECT `host:port/service_name` - not a SID. Refused on every other kind |
