...
mod tests {
    ...
    fn a_fully_measured_diff_still_states_both_numbers() {
        ...
        assert!(coverage.unmeasured().is_empty(), "the fully measured diff leaves no unmeasured added tests");
    }
    ...
    fn a_test_only_helper_is_in_neither_number() {
        ...
        assert!(coverage.unmeasured().is_empty(), "an empty added set leaves nothing unmeasured");
    }
}
