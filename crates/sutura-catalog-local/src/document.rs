...
use sutura_domain::catalog::{
    Anchor, AnchorValue, Description, Dimension, DimensionValue, InconsistentDefinitions, InvalidDimensionValue, Metric,
    Model, Relationship,
};
...
pub enum InvalidMetricDocument {
    ...
    #[error(transparent)]
    Inconsistent(#[from] InconsistentDefinitions),
...
impl MetricDoc {
    pub fn into_domain(self, description: Description) -> Result<Metric, InvalidMetricDocument> {
        ...
        let anchor = match self.anchor {
            None => None,
            Some(doc) => Some(Anchor::new(
                doc.range,
                doc.value.into_value().map_err(|cause| InvalidMetricDocument::AnchorValue {
...
";
    ...
    fn a_dimension_declared_twice_is_refused_rather_than_deduplicated() {
        ...
        assert_eq!(
            ...
            InvalidMetricDocument::Inconsistent(InconsistentDefinitions::DuplicateDimension {
