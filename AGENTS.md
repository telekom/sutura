...
else is an adapter, and nothing depends on an adapter. Each port trait arrives WITH the implementor
that makes its signature real, because a trait with no implementor is a guess at a signature and
`pub` hides it from `dead_code`. Three are here now, and they are not all the same kind of port.
`SemanticCatalog` and `Warehouse` are **driven**: the domain declares what it needs, an adapter
outside implements it, and the trait sits in `sutura-domain` or the direction reverses - `-catalog-local`
and `-exec-duckdb` brought them. `Surface` is **driving** and inverts nothing: the caller is already
outside and the implementation is the application itself, so it sits in `sutura-app` beside
`LocalService`, its one implementor. *It used to sit in `sutura-http`, which a review caught: a
driving port declared inside a transport is one every other transport has to reach through that
transport, and nothing here may depend on an adapter.* `CredentialBroker` is still absent for the
same reason it always was: nothing implements it yet, and inventing it would be worse than naming the
gap - which is why the identity invariant below says outright that it is not mechanised.
...
| `sutura-semantic` | `Query` → `QueryPlan`, and that is where `compile` stops. **It renders nothing and names no dialect**, which is now a fact about its dependency list rather than a discipline: there is no SQL generator in its tree to call. The dialect used to be an argument to `compile`, which meant it decided nothing and could disagree with the adapter that ran the statement |
| `sutura-sql` | `QueryPlan` → one statement in one dialect, plus the two decisions the dialect layer does not make for us (placeholder style, forced identifier quoting). **The only crate that names `polyglot-sql`.** It exists as a crate rather than as `sutura-semantic::generate` because a `pub` rendering module in the core put a pre-1.0 SQL generator in the transitive closure of every consumer of the core - including `sutura-serve`, which executes plans on the engine, renders nothing, and could reach none of it. It depends on `sutura-domain` and on nothing else of ours; `sutura-semantic` does not depend on it and `cargo xtask check-boundaries` fails if either edge comes back |
| `sutura-app` | The service; generic over ports, holds no framework types. Also holds the `Surface` driving port and `LocalService` - the two entry points with the ports' generics erased, for a transport whose handler is a concrete function |
| `sutura-catalog-local` / `-datahub` | `SemanticCatalog` adapters (markdown with YAML frontmatter on disk / a catalog service). `-local` exists today; `-datahub` is planned and does not exist |
| `sutura-exec-duckdb` / `-postgres` / `-clickhouse` | `Warehouse` adapters over DATA SOURCES - a place data already lives, that somebody wants to ask questions of. Each renders the plan into its own dialect through `sutura-sql` and pushes the statement down. `Warehouse` is the PORT's name and says nothing about what sits behind it. `-duckdb` exists today; the other two are planned |
...
| `sutura-arrow` | *(planned, and does not exist)* `RecordBatch` → Arrow IPC / Flight SQL. Named here because it is where a wire format would go, not because there is one: nothing leaves this process as Arrow today, `arrow` is not a direct dependency of any crate, and no crate holds an IPC writer, a Flight service or an encoder. The engine executes over Arrow *internally*, which is a different claim |
| `sutura-http` | **Exists.** Transport only, no business logic: an axum surface with a versioned `v1` tree, a liveness probe, a generated interface description, rate limiting and a bearer gate. It is where the absent identity port stopped being theoretical - **the token authenticates the DEPLOYMENT, not the caller**, so every question is still answered with whatever access the process already had, whoever asked. That sentence is printed on every boot and is in the interface description |
| `sutura-mcp` | *(planned, and does not exist)* The second transport. It is **not** the reason `Surface` exists as a trait, and the previous version of this row got that wrong: the reason is that `answer` is generic over a port with an associated error type while a request handler is a concrete function, so the erasure has to happen with one transport too. The trait is no longer in the wrong crate - it moved to `sutura-app`, so an MCP adapter imports `sutura_app::surface::Surface` and never names `sutura-http`. `sutura-http`'s `surface` module is a re-export of it |
| `sutura-config` | The settings tree, the `Environment`, and the startup refusals. Holds no framework at all, deliberately: it decides what the service may do before anything is built. Reads paths and never opens a socket |
| `sutura-runtime` | The process-global concerns a library must not install as a side effect of being linked: the tracing subscriber, the panic hook, the shutdown signal, the banner |
| `sutura-serve` | The composition root for the HTTP surface: the one place that names an adapter. Synchronous down to one `block_on`, because the engine holds its own runtime and entering one inside another aborts |
| `sutura-cli` | The binary; composes adapters |
| `xtask` | The repo gates: boundary check (dependency direction, the forbidden edges named in `FORBIDDEN_EDGES`, and the typed surface of a library crate), file-length check, unused-dependency check, line-ending check, and the CRAP score - complexity weighted by the tests that cover it. Schema dump and drift check arrive with the schemas |
...
(cranelift), because that is what makes local codegen fast, and local iteration stays there
deliberately. **CI is the authority and runs the pinned stable throughout.** Locally the split is
not uniform: every `just` task and devenv script sources `nix/stable-env.sh`, and so does the
`rust-clippy` commit hook, while the `rust-fmt`, `rust-check-changed` and `rust-doctests` hooks
run on the shell's cargo. So a bare `cargo clippy` reports lints stable has never heard of.
**Do not conclude a branch is red from one** - run `just lint`. Two exceptions worth knowing:
the `fmt-parity` push hook builds `checks.fmt` so a rustfmt divergence is caught before the pull
request, and the CRAP gate's coverage run must be stable because `-C instrument-coverage` does
not exist under cranelift. CONTRIBUTING.md has the table.
...
just crap           # CRAP score: complexity weighted by coverage
...
| A question's time range is bounded, and bounded to a size | **Two mechanisms, because the two halves are different kinds of claim.** `TimeRange` has no unbounded form and no constructor that omits an endpoint, so an *absent* bound is unrepresentable rather than refused. A *present* bound nine thousand years out is refused: `resolve` compares `TimeRange::days()` against `query::MAX_RANGE_DAYS` - 3653, ten calendar years at their longest - and returns `RefusalReason::TimeRangeTooLong`. The cap is on the caller's path and deliberately not in the constructor, because the same type is also a metric's anchor range, authored in a catalog: a maximum on the type would govern authorship in order to govern requests. `refused-range-too-long.yaml` provokes the variant from a question file, and `the_longest_permitted_range_is_answered_and_one_day_more_is_refused` in the golden suite stands on both sides of the boundary a day apart. **`plan::MAX_ROWS` is not this bound and never was:** it caps rows *returned* after the aggregate, so a question that scans all of history into one bucket is inside it. *Gap, and it is two gaps: (1) the span bounds rows scanned and nothing bounds the groups built inside it, which are the span times the cardinality of up to four dimensions - `MAX_ROWS` REFUSES that result rather than bounding the work: the groups are built, and then the answer is declined; (2) this is a per-QUESTION bound, not a per-caller budget - three permitted ten-year questions cover thirty years, and nothing correlates two requests because nothing keeps per-caller state. A real budget needs a clock, a subject and somewhere to put a counter, which is the same missing port the identity row is about* |
...
| An unvalidated bundle is never served | The service accepts only `sutura_app::Validated<PinnedDefinitions>` - anything else does not compile - and that type has exactly one constructor, `sutura_app::verify_and_validate`, which takes the `Warehouse` and calls it. **The previous version of this row was overstated and a review caught it.** The proof used to be `Validated::new(pinned, &report)` in the domain, beside a public `AnchorReport::new`, `AnchorReport::record` and `AnchorCheck::Matched` - so any caller could enumerate the bundle's anchors, record `Matched` for each without opening a data system, and get a bundle the service would serve. The golden suite did exactly that. What holds now is the module boundary: `Validated`'s field and its tuple constructor are visible only inside the private module that also holds the verifying operation, so the forgery is a compile error rather than a convention, and a `compile_fail` doctest on `verify_and_validate` is the check. The domain keeps the rule as `AnchorReport::verdict`, which returns `Result<(), NotValidated>` and no bundle. *It still cannot prove WHICH data system: `W` is a port, and a fake is what the golden suite passes deliberately. `answer` narrows it by refusing a plan whose source is not the adapter's own.* The anchors re-run in CI and at startup, and failure fails readiness |
...
| Adding a metadata provider or a data system is a registration, not a test edit | The test suite is a **matrix over `crates/sutura-app/tests/adapters/mod.rs`**, which is the only place an adapter or a dialect is named. A registration is one `impl CatalogUnderTest` or `impl DataSystemUnderTest` - what it is called, and how to open it over the fixtures - plus one line in the `registered!` macro. The golden corpus, the refusal corpus, the anchor check, `dry_run` acceptance and the engine comparison in `differential.rs` are then each expanded once more, with no test body touched, because a cell macro expands per entry and the bodies are generic over the port. Generics with trait bounds rather than `dyn`, which is the convention and is also forced: both ports carry an associated error type. Each artefact sits on the axis that decides it - a plan on the catalog axis, a statement on the dialect axis, rows on the data-system axis - which is what stopped three byte-identical copies of every plan snapshot being kept, one per dialect. `every_dialect_the_renderer_supports_is_registered` compares the dialect arm against `sutura_sql::dialect::ALL`, so a render target added to `sutura-sql` with no golden fails rather than reading as covered. *There is no equivalent gate for the two adapter axes, because there is no list for one to compare against: an adapter is a crate, and a crate nobody registered is a crate nobody built.* **And this is a claim about the TESTS, not about deployment:** which adapter runs is still decided at compile time in `sutura-cli`, and no configuration names one - see `docs/architecture.md` |
...
| A join cannot silently change a measure | **Half mechanised, and the half that is missing is the one that matters.** `Definitions::assemble` refuses a dimension reached through a relationship whose *declared* cardinality may duplicate the metric's rows, and `a_dimension_join_does_not_change_the_measure` reconciles the grouped rows against the ungrouped total - which is what caught a real INNER-join bug on this branch (470023 against 570022). **What nothing checks is whether the declaration is true.** `may_duplicate_rows()` is a pure function of the `JoinType` a catalog document wrote down; a relationship declared `many_to_one` whose target key actually has duplicates fans the fact rows out, `assemble` accepts it, and anchor verification cannot see it because an anchor is asked with no dimensions and therefore no join. A `sum` over duplicated rows is a wrong number that raises no error anywhere. *Gap: catalog cardinality is a TRUSTED, externally enforced precondition today. The honest mechanism is a target-key uniqueness probe at readiness, before serving any metric that traverses the relationship - the data is only reachable through the `Warehouse` port, which is where anchors already re-execute* |
| A result that hit the row cap is refused, not truncated | `QueryPlan::row_limit()` is `max_rows + 1`, so a result at the cap is distinguishable from a result cut off by it, and `answer()` returns `RefusalReason::ResultTooLarge` rather than rows. A refusal is the honest outcome: the question is too wide to certify, which is a governance answer and not an error. The check is ONE comparison above the port rather than one per adapter, so it cannot drift between them - but the `+1` request is per-adapter and that is where the hole was, so both legs are pinned: the SQL path by 39 goldens reading `LIMIT 10001`, the engine path by `the_engine_asks_for_one_row_past_the_cap_exactly_as_the_sql_path_does` asserting on the logical plan. Three tests stand on the boundary - over the cap refuses, exactly at the cap answers, and the adapter was asked for cap+1. *Not in `PROVOKED`, and it cannot be: that table is refusals a question file provokes before anything runs, and this one is decided after a data system answered* |
...
| The SQL generator is not in the compiler's closure | `FORBIDDEN_EDGES` in `xtask/src/boundaries.rs`, the second half of the same check: `sutura-semantic` may reach neither `polyglot-sql` nor `sutura-sql`, over the whole transitive tree and with `--all-features`, so hiding the edge behind an innocuous crate or behind a feature fails it too. **This row exists because the previous arrangement was a comment.** `compile` already stopped at a `QueryPlan` - the `Warehouse` port carries a plan, and the engine adapter renders nothing - but `generate` and `dialect` were `pub` modules OF the core, so `sutura-serve → sutura-http → sutura-app → sutura-semantic → polyglot-sql` linked a pre-1.0 SQL generator into a binary that renders nothing and can reach none of it. Two entries rather than one, and the second is the load-bearing one: adding `sutura-sql` to the core's manifest to share a type would restore the first edge transitively with no line anywhere naming `polyglot-sql` for a reviewer to see. An ALLOWLIST like the domain's would be the wrong shape here - it would mean enumerating a legitimate tree of thirty crates to state a fact about one - and each entry carries the reason and the fix, which the failure prints. *What it does not check: that `sutura-sql` is the only crate rendering. Nothing stops a second generator; what stops it is that there is one place to put one* |
| A driving port is not owned by one of its callers | `Surface`, `SurfaceFailure`, `ServiceNotStarted` and `LocalService` are declared in `sutura-app`, so a second transport imports `sutura_app::surface::Surface` and never names `sutura-http`. **Not mechanised, and this row says so.** The boundary check reads the direction of dependencies and cannot read *which crate a trait is declared in*; what holds this up is that `sutura-app`'s dependency table is unchanged by hosting it - `cargo tree -p sutura-app -e normal` is `sutura-domain`, `sutura-semantic`, `thiserror` and nothing else - so the move could not have smuggled a framework in, and that half IS the gate's. *A review found the previous arrangement: the trait was in `sutura-http` while its own module comment said an MCP transport would consume it, which would have made one transport adapter depend on another. `sutura-http`'s `surface` module is now a re-export, and the follow-up that deletes it points `state.rs`, `routes/` and `harness.rs` at the canonical path* |
| A newtype's invariant cannot be walked around | The field is private and the constructor is the only way in, so a violating value is unrepresentable rather than merely rejected. The typed-surface half of the boundary check fails a `pub` field on a `pub struct` in a library crate. `serde` is routed through the constructor with `#[serde(try_from = ..)]`, because a derived `Deserialize` writes past it - *Gap: that routing is not itself checked; review catches it until a gate does*. *Second gap, and it has cost us once: the gate reads one declaration at a time, so it cannot see a SECOND public path that reaches the same value with the invariant unmet. `Validated<PinnedDefinitions>` was exactly that - its own field private, and a public `AnchorReport` next to it that a caller could fill in by hand - and a newtype whose invariant is walkable through another constructor is worse than no newtype, because the name is what a caller trusts. Reading a newtype means asking what else in the crate's public surface can produce its argument* |
...
| Complexity in the invariant core is covered by tests | `cargo xtask crap`, in the hooks and in CI. CRAP is `CC^2 * (1 - coverage)^3 + CC`, so it fails on the *combination* that neither a complexity limit nor a coverage percentage catches alone. **Scoped to `sutura-domain`**, and the scope is the honest part of the row: coverage is a separate compile profile, so `--workspace` costs over 80 CPU-minutes and never reached a test, while a per-crate run only sees that crate's own tests - which for `sutura-domain` is its whole test suite and for `sutura-semantic` is not, so adding that crate produces seven 0%-coverage artefacts rather than seven findings. *Gap, stated because it is the whole of what this does not cover: every dialect renderer, the resolver, the planner, the catalog loader and both adapters are unscored.* The ratchet is the `threshold` number in `.cargo-crap.toml` and not a downloaded baseline - the Nix sandbox has no network and no `.git` - so it answers "is anything over the line", never "did anything get worse". Both fail-open modes are refused by name: an LCOV with no source records, and a report with no entries or a scoped package that contributed none. `docs/crap.md` has the measurements |
...
| A result cannot be separated from what defined it | **Half mechanised, and the half this row used to claim does not exist.** There is no Arrow on the way out and there are no wire envelopes: `Warehouse::execute` returns a `RowSet` - `Vec<String>` columns and `Vec<Vec<Value>>` rows - `arrow` is not a direct dependency of anything here, and no crate holds an IPC writer, a Flight service or an encoder. The engine executes over Arrow *internally*, which is a different claim, and `sutura-arrow` in the Layout table above is a crate that does not exist yet. What IS enforced today: `ToolOutcome::Answer` carries `Provenance` as its own typed field beside the rows, built from the bundle that answered - there is no constructor that merges the two or that omits the provenance - and `PinnedDefinitions::pin` computes the digest FROM the definitions it stores rather than taking one beside them, so a bundle cannot carry provenance for content that did not produce it. *A review found the earlier hole: the constructor took any digest next to any `Definitions` and said in its own comment that the two need not be related. What remains outside the type's reach is a digest function that ignores its argument - one function, in one adapter.* When a wire format lands, this row gets the encoder half back, with the byte-compare that holds it up |
| Every call is attributable, refusals included | **Not mechanised, and this row says so rather than crediting something that does not exist.** There is no `AuditSink` - no trait, no adapter, no call site - and no logging dependency in the workspace at all, not even `tracing`. A refusal today is a `ToolOutcome::Refusal` value handed back to the caller and written down nowhere; so is an answer. The narrow claim that holds is that an outcome is *representable* as a record: `ToolOutcome` is an enum whose refusal branch carries a typed `RefusalReason`, and `ToolOutcome::refusal` exists for a sink to read, so nothing is a formatted sentence by the time it would be recorded. What would enforce the row is a port plus the adapter it arrives with, and by this file's own rule a port does not land before its adapter. Until then there is no principal chain to record either - see the row above on running as the calling principal |
