...
answered from physical structure rather than a certified metric - [its own README](raw-sql/README.md) says what that costs and what it does not cover. The
sections below say which is which for the first three; `raw-sql/`'s own page is
...
the model. `crates/sutura-catalog-local`'s
`the_authored_sql_example_loads_and_its_fragment_is_under_the_digest` pins the fragment's digest;
`crates/sutura-app`'s `the_authored_sql_example_does_not_boot_against_the_in_process_engine` drives
the whole directory through the composition path and asserts the refusal. Its own
[README](authored-sql/README.md) says what is and is not proven.
...
keys look like `C0001` because a generator wrote them, `authored-sql/` is four rows written by hand across two files, and no row corresponds to a real
...
`single-player/` is CSVs the engine reads directly, `authored-sql/` is refused before any of its rows
are opened, and `multi-player/`'s runnable form is the
