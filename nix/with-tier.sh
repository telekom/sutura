...
#
# ONE definition, sourced by every venue that runs the suite locally: `just test`, `just gates` and
# `nix/run-gate.sh tests` (which the commit and push hooks both reach). The nix sandbox does not
