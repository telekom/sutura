...
| Protected-resource discovery in `direct` mode | `sutura_http::routes::protected_resource` and `sutura_http::inbound::gate` | One startup value owns the public RFC 9728 route, its document and the absolute URL in the `resource_metadata` challenge parameter. The document publishes the exact audience as `resource` and the exact configured issuer in `authorization_servers`; the validator reads that same token requirement. The route is outside `/v1` and capability authorization, inside the probe limiter, and absent in `behind-gateway` and single-player deployments |
