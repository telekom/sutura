...
Status: **accepted; the first slice lands the record, the port signature and the refusal - every
adapter accepts the deadline and ignores it. Nothing stops on it yet**; the engine, BigQuery and
Postgres slices that make an adapter honour it follow behind `telekom/sutura#160`. Four accepted
