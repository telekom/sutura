...
| --- | --- |
| A mono answer resolves to one data system | The plan stage's source set. One is a mono plan; exactly two are split into a fact leg and a lookup leg (`answer` executes them where the registered adapter declares `EXECUTES_LEGS` - this engine does - and refuses as `FederationNotExecutable` where it does not); three or more refuse as `PlanSpansTooManySources`. A second data system reachable from one process is federation once it is split, and does not merge with a mono plan by being convenient |
