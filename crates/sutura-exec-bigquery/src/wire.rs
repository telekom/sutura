...
// PUBLIC, not private - the same reason `bounds`/`credential`/`document` are: a `pub use` out of
// a PRIVATE module reaches the generated API page as an undocumented `use None` stub (measured on
// `pub use sts::StsOverHttp`, this file's own header), and `WireAgent` is too central a type to
// lose its methods' docs to that.
pub mod agent;
pub mod bounds;
mod budget;
pub mod credential;
...
// [`WireAgent`] moved to `wire/agent.rs` for the file-length gate - see that file's own header.
