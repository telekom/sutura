...
// The deployment's identity fixtures, in their own file for this file's `max-lines` reason;
// re-exported here so every `crate::testing::…` path is unchanged.
#[cfg(test)]
mod identity;
pub(crate) use identity::{ISSUER, KID, RESOURCE};
pub(crate) use identity::{accepted_by, an_issuer, direct_overlay, every_scope};
