...
A data system's driver is a dev-dependency. `sutura-cli` links the engine only in its DEFAULT build,
which is what keeps the musl artifacts building - nixpkgs has no musl `libduckdb`. `nix/duckdb.nix`
is the single path from nixpkgs to that library, imported by `flake.nix` and `devenv.nix` alike.
