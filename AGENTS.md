...
| We never re-parse SQL we did not generate | The dialect layer's `transpile` feature is not compiled, so a call to it does not build. NOT a lint: an unresolvable path in `disallowed-methods` is silently ignored by clippy, verified, so such an entry would read as enforcement and do nothing |
