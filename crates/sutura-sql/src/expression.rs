...
fn check(
    ...
    // #589: the pinned `polyglot-sql`'s `Parser::parse_data_type` used to not terminate on an
    // unclosed parenthesis (`a.:S1(`), which was refused here rather than handed to the parse.
    // Fixed upstream by `tobilg/polyglot#447`, in the release this workspace is now pinned to -
    // measured against every recorded fuzz artifact for this defect. The superlinear-parse
    // ceiling `docs/adr/0004` records is a *different* limit and does not come out with this
    // guard; see that document for what still holds.
