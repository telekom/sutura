...
| Physical structure is not a certified metric           | **The pinned definitions.** Present only when the bundle has models and has no metrics; it explains authored prose and the semantic promotion path, and names a table or column only where `prompt.physical_schema: listed` (`docs/adr/20260924090917-listing-a-physical-schema-with-no-certified-metric.md`) - never a description or a type |
...
| `prompt.physical_schema`   | `omitted` | Only consulted where the bundle has zero certified metrics. `listed` names every model's and column's NAME (never a description or a type) in the onboarding ramp; `omitted` says the structure and the prose exist and names neither |
