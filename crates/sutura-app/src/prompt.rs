...
use sutura_domain::capabilities::{DeclarableKind, DefinitionKind};
use sutura_domain::catalog::{DimensionValue, Metric};
...
// `knowledge` has one, and `guidance` is re-exported flat so `prompt::guidance` stays the path
// a composition root prints from - the BigQuery adapter's `wire/bounds.rs` states why the module
// is public rather than private.
pub mod refusal;
pub use refusal::guidance;
use refusal::refusals;
...
///
/// The trigger reads both halves of the bundle's claim: actual non-empty structure with no metric,
/// and a reached contributor whose digest-covered declaration provides `Structure`. It does not key
/// on an adapter or source name, so every declaring catalog that produces this shape gets the same
/// guidance and none can opt into it with a suggestive label.
fn physical_schema_guidance(pinned: &PinnedDefinitions) -> String {
    let definitions = pinned.definitions();
    let declares_structure = pinned.manifest().entries().values().any(|contribution| {
        contribution.reached()
            && contribution
                .capabilities()
                .declares(DeclarableKind::Definition(DefinitionKind::Structure))
    });
    if definitions.models().is_empty() || !definitions.metrics().is_empty() || !declares_structure {
        return String::new();
    }
    ...
    if metric.dimensions().is_empty() {
        ...
        for (name, dimension) in metric.dimensions() {
            let how = dimension.allowed_values().map_or_else(
                ...
                |values| {
                    let listed: Vec<&str> = values.iter().map(DimensionValue::as_str).collect();
...
///
/// **What this function does NOT give you is [`quote`]'s column-zero property, and the difference is
/// exactly `indent`.** Re-flowing on whitespace means an author's own newline cannot survive, which is
/// most of the way there; the rest is that every line after the first begins with `indent`, so an
/// author's word starts a line at column zero whenever `indent` is `""`. That is fine for the
/// literals in this file, which are ours, and it is not fine for any text a catalog supplied - a line
/// beginning `##` is a heading to every markdown reader whatever preceded it. So: **a caller passing
/// text that came from a catalog passes a non-empty `indent`**, and `knowledge::caveats_about` records
/// why it is the call that had to change.
...
/// so the filter above sees none of them, deliberately. **Every channel into this function is now a
/// parsed type**, which is a claim this comment could not make before and is worth writing as the
/// enumeration rather than as an assurance:
///
...
/// * a metric's description, at the one call site below. It **was** a plain `String` on
///   `sutura_domain::catalog::Metric`, built from a markdown document with no character check
///   anywhere on the path, and it was therefore the one channel whose rendering could differ from its
///   content - a paragraph reading `status = 'active'` in every terminal and every diff, saying
///   something else, under a digest taken over text nobody read. It is a
///   `sutura_domain::catalog::Description` now, which refuses the set at parse the way a note body
///   does, so the guard is at the type boundary and not in this filter.
///
