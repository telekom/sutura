...
const TASKS: &[Task] = &[
    ...
    Task {
        ...
        // `AGENTS.md` would call a wish - here, `docs/where-identity-is-proven.md`'s own "a new venue
        // arrives as a row with its exclusions written" - held by nothing but recall. Text in, verdict
        // out, no tree evaluated, so `hygiene`.
        name: "check-venues",
        description: "every identity acceptance task is named by docs/where-identity-is-proven.md",
        kind: Kind::Hygiene,
        run: venues::run,
    },
    Task {
