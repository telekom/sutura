...
///
/// A function rather than a body inside one constructor, because two types are held to it and a
/// second copy of a character rule is a second thing to keep true. [`crate::text`]'s own module
/// documentation is about exactly that drift, and it exists because it happened once already to a
...
/// characters is three times the longest decimal an `i64` can render, so nothing a data system can
/// return as a scalar is refused by the length alone.
///
...
// private field, and both adapters carry this value in a document they deserialize.
#[serde(try_from = "String")]
