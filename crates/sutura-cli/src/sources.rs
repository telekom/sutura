...
/// The CLICKHOUSE half of this module: one declared HTTP endpoint, secured as the source declares.
///
/// **Its own file for the reason `bigquery`'s is** - `cargo xtask max-lines` fails at 1000 lines
/// rather than warning. It is thinner than its two siblings on purpose: the BUILD lives once in
...
fn from_the_registry(
    ...
    match configured.kind() {
        ...
        sutura_config::SourceKind::ClickHouse => {
            if let Some(given) = data {
                return Err(format!(
                    "`sources.{source}` is a ClickHouse database, and {} was given on the command \
                     line as a data directory - a database has none, so the argument selects nothing. \
                     Drop it; the database that entry declares is what will be read",
                    given.display()
                ));
            }
...
///
/// It is the number a job's deadline is derived from, so a change to `defaults.yaml` reaches the
