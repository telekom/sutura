...
///
/// Both adapters deserialize this type - `sutura_catalog_local`'s `AnchorLiteral::Text` and
/// `sutura_catalog_datahub`'s `SuturaAnchor` - so the `try_from` is what makes the character rule
/// reachable from a catalog file at all. Same assertion, and same reason, as the declared value's
/// above.
#[test]
