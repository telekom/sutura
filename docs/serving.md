...
| `deadline_exceeded`                | `422`  | Narrow the period, group by fewer dimensions or add a filter. This deployment stopped the question after its configured budget (`server.request_timeout_seconds` minus a one-second margin); the sentence names it. `docs/adr/0029` records the shape - the in-process engine stops at cooperative yield points, the default-off Postgres source uses `SET LOCAL statement_timeout`, and BigQuery sends what is left of the budget as the job's own `jobTimeoutMs` so the service ends the job too. On BigQuery this refusal is the ALREADY-SPENT half only: a job the service ends at its own bound arrives as an opaque driver error and is reported as a source failure, not as this refusal |
...
the deadline as the job's own time bound** - the ADBC driver's `bigquery.query.job_timeout`, which is
the service's `jobTimeoutMs`, so the service ends the job at that bound whether or not this
deployment is still waiting; a request whose deadline is already spent is refused before the driver
is loaded rather than submitted with no bound. Two limits: it is the service's clock rather than this
one's, so a job may end slightly late, and work already done is already billed. The concurrency bound
is therefore load-bearing rather than belt-and-braces here: it caps how much work can accumulate
where cancellation is delayed, cooperative, or bounded only at the other end.
...
statement stop; for BigQuery it is whenever the driver answers, which the job's own `jobTimeoutMs`
bounds at the service but this process cannot shorten once submitted. The backlog is therefore
bounded even when a timed-out caller cannot stop the underlying work itself.
...
| `server.port`                                   | `8080`                               |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `server.request_timeout_seconds`                | `30`                                 | At most 300. Bounds a caller's whole wait on **both** surfaces - the `408` here, and a tool result on the agent surface. Minus a one-second reply margin, it also opens the shared execution deadline: the engine observes it at cooperative yield points and Postgres after its execution-lock wait. On BigQuery what is left of it is sent as the ADBC driver's `bigquery.query.job_timeout`, so the service ends the job too; a spent deadline is refused before the driver is loaded. **What an operator has to know:** that is the service's clock, not this one's, so a `408` here and the job ending there are not the same instant, and a boot-path call carries no time bound at all |
...
| `catalogs[].kind`                               | `markdown`                           | Which adapter opens this catalog. `markdown` or `datahub`; a build without `--features datahub` refuses the latter by name, naming the feature. **A deployment declares catalogs of ONE kind** - a `markdown` and a `datahub` entry together are refused, not merged; see `docs/adr/0016`                                                                                                                                                                                                                                                                                                                                                                                                     |
| `catalogs[].dir`                                | `catalog`                            | `markdown` only for its meaning; **required non-empty for every kind, including `datahub`**, which does not read it - a stated limit of the settings surface rather than a per-kind field. At most 1,000 documents / 16 MiB of documents in total; more is a startup refusal naming the bound; raising it is a source change, not a setting                                                                                                                                                                                                                                                                                                                                                   |
| `catalogs[].data_dir`                           | `data`                               | `markdown` only for its meaning; **printed by the startup banner and read by nothing that opens a data system** even for a `markdown` catalog - a served source's files come from its own `sources.<alias>.data_dir`, and the `sutura` command reads that same entry or else the directory on its command line. Required non-empty for `datahub` too, unread, the same stated limit as `dir` above                                                                                                                                                                                                                                                                                            |
...
reaches all three data systems** - the in-process engine at a cooperative yield, Postgres as
`statement_timeout`, and BigQuery as the ADBC driver's `bigquery.query.job_timeout`, which is the
service's own `jobTimeoutMs`. On BigQuery it is what is LEFT of the deadline at submit, and a request
whose deadline is already spent is refused before the driver is loaded rather than submitted with no
bound. **Two things it does not cover:** the boot-path calls a deployment makes for itself
(`verify_anchor`, the identity read) have no caller deadline to derive a bound from, and work the job
has already done is already billed when the service ends it - `max_bytes_billed` below is the
separate bound on that. `docs/adr/0029`'s third amendment is the record.
