...
- The first inbound dependency that must **verify** a signature enters the tree. `sutura-http` presents a
  ...
  that needs it.
- The transport grows a challenge, protected-resource metadata it serves about itself, and a validator. All of
  it is transport: it parses a wire shape and produces a domain value, and it decides nothing about what a
  question may ask.
...
| Protected-resource discovery in `direct` mode                                      | `sutura_http::routes::protected_resource` and `sutura_http::inbound::gate` | One startup value owns the public RFC 9728 route and its document. The document publishes the exact audience as `resource` and the exact configured issuer in `authorization_servers`; the validator reads that same token requirement. A challenge includes the absolute metadata URL only when an origin-form request's raw `Host` and request target reproduce that exact resource, because RFC 9728 section 3.3 makes mismatched metadata unusable. Absolute-form targets are not matched: the HTTP URI parser canonicalises standard schemes, so a match there would compare against a normalised spelling rather than the configured identifier's exact bytes. The route is outside `/v1` and capability authorization, inside the probe limiter, and absent in `behind-gateway` and single-player deployments                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
2. **Client registration and client authentication.** Decision 2's sub-sections 1 and 2 are decisions
...
3. **A ceiling derived from a scope.** Decision 4's claim shape exists - `Scopes`, parsed and bounded,
...
4. **Binding a gateway assertion to a request, and any record of what has been seen.** Added by
