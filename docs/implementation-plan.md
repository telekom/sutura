...
| 22 | ~~`build/supply-chain`~~ | nothing - orthogonal | **DONE** - [0021](adr/0021-how-a-published-artefact-proves-where-it-came-from.md). A Sigstore bundle per asset, `cosign` on all six image references, CycloneDX and SPDX per leaf image from the auditable binary, SLSA provenance, ORT in `licence-review.yml`, and REUSE. **One piece of the roadmap bullet is NOT built:** the licence report is published as a CI artifact and is not signed |
