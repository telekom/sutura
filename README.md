...
uncertified question is not refused so much as unsayable.
...
skeleton. What works today is the environment, the release pipeline, and the checks that keep
the guarantees above from quietly becoming aspirations. The query path is not built yet.

## Documentation

Start with the [documentation](https://telekom.github.io/sutura/) — how to get set up, what
the pieces are, and why they are that way. It is built from the `docs/` directory in this
repository.
