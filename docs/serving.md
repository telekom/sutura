...
first half does not survive checking: nothing mainstream retries a `4xx` by default, and `422` is
documented the other way round, as a status a client should expect to fail again on an unchanged
request. What the `200` did cost was legibility to everything that reads
