...
| `security.credential_cache.enabled`             | `false`                              | An exchanged credential (a `bigquery` source's `impersonation-at-source` leg) cached per subject, per process. Off because nobody has measured the round trip it saves - see [ADR 0031](adr/0031-caching-an-exchanged-credential.md)                                                                                                                             |
