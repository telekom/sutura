...
A markdown document carrying one row per third-party crate: the name, the resolved version, and the
SPDX expression that crate's own manifest declares. It is generated by `cargo metadata`, it is
published with every release as `sutura-attribution.md`, and **it is not committed.**

#### AMENDED: it was committed, and being committed is what it cost

**The original decision was to commit it, and that decision is reversed.** The argument for
committing is kept here because it is a good argument that was defeated by a cost it did not
anticipate, and *that* is the part worth reading:

> Committed rather than release-only, and the *Canonical Sources* rule in `AGENTS.md` is why: one
> owner, regenerated rather than hand-edited, and the regeneration checked. A release-only document
> is none of those - it has no owner a reviewer can open, and nothing can fail when it falls behind.
> The release still publishes it, as a copy, which is a different thing from generating it there.

What that missed: **a committed derived file has TWO owners, not one.** The generator and the
commit. Keeping them in step needs a staleness gate, and a staleness gate can only be discharged by
whoever can run the generator - which Dependabot cannot, because it opens pull requests under a
read-only token. So the file went stale on every dependency bump, the gate went red, and no bump
could reach green without a human pushing to the bot's branch. Measured on two open bumps in
`github.com/telekom/sutura#431`; `#462` established that the artefact discharges a real obligation
and that its committed form is the part to drop.

Reversing it makes the generator the only owner, which is what *Canonical Sources* asked for in the
first place. **The release generates the asset from the `Cargo.lock` of the tag it is publishing**,
so the copy step that could hand a stale document to the signer is gone, and the chain from
resolution to signed notice is shorter than it was.

**Three mechanisms, and the split is which input each one needs.** `cargo xtask
check-attribution-owner` reads a path's ABSENCE and the release workflow, invokes nothing, and so
runs on every commit and inside the nix sandbox: it holds that no committed copy comes back and
that the release still generates. `cargo xtask check-attribution` generates one and refuses an
incomplete result - it needs a resolvable registry, which is why it is a `just gates` and `ci.yml`
step rather than a hygiene gate; `check-api-docs` is the precedent and the reason has the same
shape, an input the sandbox has not got. The release's own count floor asserts the generated BYTES
name at least a hundred crates.

**It cannot byte-compare any more, and what replaced that is a different kind of oracle.** With
nothing committed there is no golden to compare against, so `check-attribution` triangulates
instead: the package SET comes from `Cargo.lock` parsed as text and the LICENCES come from `cargo
metadata`'s resolution, and a disagreement between the two independent inputs is the failure. It
also refuses a crate that declares no licence at all - which the old generator printed to stderr
while writing `NOT DECLARED` into the document and exiting zero.

**What the reversal LOST.** A new dependency's declared licence no longer appears in a pull-request
diff. That was real and nothing here restores it. `cargo deny check` still refuses a licence off
the allowlist and the gate above refuses an absent one, so the policy floor did not move; what is
gone is a human noticing a licence change from one permitted value to another.

**A REVIEW'S CORRECTION is why the offline half can never be the whole gate, and it outlived the
committed file.** An offline check over a document's text can only see that a licence cell is
non-empty, so changing any row's SPDX expression to arbitrary text passed it: the main content of a
generated artefact was trusted rather than compared, which is the one thing the *Canonical Sources*
rule forbids. Not committing the document removes the hand-edit entirely - there is no text for
anybody to change - but it does not remove the case the crate key cannot see at all: a git
dependency moving to another revision, changing its declared licence while keeping its name and
version. That is still only reachable by a check that resolves, which is why `check-attribution`
needs a registry and the sweep's half does not.
...
`cargo auditable` section, read back by `syft`; the attribution document is the obligation.
...
- A generation names every third-party package in `Cargo.lock` - registry, git and vendored path
  alike - at the resolved version, names nothing else, and carries a declared licence for each one.
  That is `cargo xtask check-attribution`, in `just gates` and in `ci.yml`.
- No committed copy of it exists, and the release still generates the asset. That is `cargo xtask
  check-attribution-owner`, in the hygiene sweep and therefore in `just validate`. **An absence that
  nothing witnesses silently returns**, which is why it is a gate rather than this sentence.
...
The attribution document owns the checked workspace-wide declared-licence list, and Syft owns the two
