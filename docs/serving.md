...
| `legs_decide_identity_differently` | `409`  | Ask the same metric without the dimension on the second data system. The two data systems decide who is asking in two different ways, and a total made of rows read under two identities is a number neither is entitled to. No published build can reach it, and not because every linked adapter serves everyone as one identity any more - `bigquery` does not - but because it is the one adapter that can carry a per-subject credential and it does not declare `EXECUTES_LEGS`, so a federated question refuses it as `FederationNotExecutable` before two postures could ever be compared |
