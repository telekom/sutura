...
| `catalogs[].kind`                               | `markdown`                           | Which adapter opens this catalog. `datahub` parses; no shipped binary links the adapter, so it is refused by the composition root instead                                                                                                                                                                                                                        |
| `catalogs[].dir`                                | `catalog`                            | at most 1,000 documents / 16 MiB of documents in total; more is a startup refusal naming the bound; raising it is a source change, not a setting                                                                                                                                                                                                                 |
| `catalogs[].data_dir`                           | `data`                               | **Printed by the startup banner and read by nothing that opens a data system.** A served source's files come from its own `sources.<alias>.data_dir`, and the `sutura` command reads that same entry or else the directory on its command line                                                                                                                   |
...
model's `source:` names.** `catalogs[].data_dir` is no longer where a served source's files are found:
it is the `sutura` command's own data directory and stays that. A deployment that declares no source
does not serve, because the catalog names a source with no entry and the process refuses before a
listener is bound.
