...
pub(super) fn provision(
    ...
    let args = teardown::up_args(names);
...
fn remove_selected_volume(root: &Path, project: &str, logical: &str) -> Result<(), Verdict> {
    ...
    match volume_selection(&found, &expected) {
        Ok(None) => {
...
///
/// The logical volume is an explicit lifecycle input so this function cannot silently apply a
/// demo-specific volume name to an unrelated selected service.
pub(super) fn down_scoped(
    scope: &Scope,
    root: &Path,
    profile: &'static str,
    services: &[&str],
    ...
    println!("  volume   {project}_{logical_volume} (only with this project's two Compose labels)");
    ...
    let removal = with_selected_endpoints_forgotten("dev-down", scope, services, || {
        ...
        remove_selected_volume(root, &project, logical_volume)
    });
...
#[cfg(test)]
mod volume_tests {
    use super::volume_selection;

    #[test]
    fn a_wrong_label_result_refuses_without_selecting_it() {
        assert_eq!(
            volume_selection(&["other_project_demo-webui-data"], "mine_demo-webui-data"),
            Err(vec!["other_project_demo-webui-data"])
        );
        assert_eq!(
            volume_selection(&["mine_demo-webui-data"], "mine_demo-webui-data"),
            Ok(Some("mine_demo-webui-data"))
        );
        assert_eq!(volume_selection(&[], "mine_demo-webui-data"), Ok(None));
    }
}
