...
1. **`Caller { subject, assertion }` is not what the port takes.** It takes the `RequestContext`, which
   ...
   **And the assertion field is deliberately absent, which is the sharper half.** Part 2 has the broker
   exchanging the caller's own token, so it gave `Caller` a `Secret`. [How a caller proves who it is](0014-how-a-caller-proves-who-it-is.md) Decision 3 then decided that the exchange chain differs
