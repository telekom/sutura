...
| 21 | `feat/demo-tasks` | 14, and one example to demo | **#595**. The tutorial and the docs cleanup it used to belong with have landed - the site's `nav` is an arc with all three plan pages excluded from it, `docs/getting-started.md` is the guided path and `crates/sutura-cli/tests/documented.rs` runs every command it prints, and `examples/multi-player` is reached by a test - so #130 is closed and this row is the half that was left. `crates/sutura-cli` links `sutura-mcp`, so the blocker is spent; what is missing is a `just demo` recipe and a chat client |
