...
because it parses the file as a workflow. `ci.yml`'s own `shellcheck` pass globs `*.sh`, and a
`run:` block is not one. So the release path's `cosign` invocations were shell nothing had ever read.
