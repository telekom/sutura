...
/// Every workspace member's directory, repo-relative, derived from `cargo metadata`.
///
...
/// hardcoded list works today and silently stops covering a crate the day one is added. The
/// directory comes off each package's `manifest_path` rather than from the `members` array in the
/// root manifest, because that array holds globs and cargo is the authority on what they expand
/// to.
pub(super) fn member_dirs(root: &Path) -> Result<BTreeSet<String>, String> {
    // CANONICALISED on both sides before the prefix is stripped: `crate::repo::root` comes from
    // git and `manifest_path` from cargo, and a symlinked checkout - `/tmp` on darwin is
    ...
    for package in packages {
        ...
        let owned = Path::new(manifest)
            ...
            .canonicalize()
            .unwrap_or_else(|_| Path::new(manifest).with_file_name(""));
        let rel = owned
            .strip_prefix(&root)
...
pub(super) struct Scan {
    ...
    ///
    /// The denominator, and it has to come from the other side or comparing it proves nothing:
    /// [`in_scope`] filters the same listing `super::run` iterates, so a walk that stops early
    /// moves [`Self::read`] and leaves this where it was. Without it the verdict was an
...
mod tests {
    ...
    use super::{in_scope, is_under, member_dirs};
    ...
    fn the_derived_member_list_holds_this_crate_and_not_the_vendored_ones() {
        ...
        let dirs = member_dirs(&root).expect("cargo metadata reads this workspace");
        assert!(dirs.contains("xtask"), "{dirs:?}");
        assert!(
            dirs.iter().all(|dir| !dir.starts_with("vendor/")),
            "a vendored package is not a workspace member: {dirs:?}"
        );
    }
}
