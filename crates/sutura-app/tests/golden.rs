...
//!   hand-built fixture rather than one the splitter produced, and it says so. **Its Postgres cells
//!   pin the leg text a release sends, and no cell on this axis executes one** - that is
//!   `sutura-exec-postgres`'s conformance binding; the engine's leg path renders nothing, and its
//!   evidence is `sutura-exec-datafusion`'s binding and the two-engine pass in
//!   `differential/federated.rs`. `golden/qualified.rs` is a third file on the
