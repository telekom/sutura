...
| A gate cannot be hung by a SECOND, unbounded subprocess wait in the compose tier | `check-bounded-wait`. One function waits on a container-runtime child, carrying a deadline; this gate holds the half that function cannot state about itself. Over `xtask/src/compose.rs` and the Rust under `xtask/src/compose/`, read with comments and multi-line string interiors blanked, only ONE file may wait: the docker module, **discovered rather than named**, so the waiter may move inside it - plus declared allowances each carrying why a bound would be wrong (`lock.rs`'s `lsof` is the only one). `disallowed-methods` cannot express this: an entry is workspace-wide and `xtask` waits on `git`, `cargo` and `nix` unbounded on purpose. **Started green.** Fails closed five ways: an empty scan, an in-scope file it cannot read, no waiter, two waiters, and an allowance that has stopped waiting | **LOCALITY, not boundedness** - it says no second wait was written here, not that every child is bounded. Per FILE, so a wait added beside the bounded loop INSIDE the waiter is invisible, and that file today holds two documented unbounded calls next to it. A wait laundered through a helper in another module is invisible, and so is one in a directory nobody put in scope. Single-line string interiors are NOT blanked, so a one-line literal naming one of the five calls is a live anchor - none is in scope today. Line-oriented, so a call split across two lines reads as none; the formatter is what owns that shape |
