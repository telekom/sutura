...
Status: **accepted; the record, port signature and refusal have landed. The engine and Postgres now
stop their certified paths on the deadline; BigQuery still accepts and ignores it. Postgres's raw
SQL path has no per-request deadline and remains bounded by its connect-time ceiling.** Four accepted
