...
//! [`crate::warm_start::pairing::TAKING`] argument name - not a line, not a file, and not an occurrence of the word `purge`.
...
//!    Since that is the only [`crate::warm_start::pairing::PHASE`] binding permitted in the artifact flow - see
//!    [`crate::warm_start::pairing::in_the_flow`] for why the rule is scoped and derived - no consumer can displace it.
...
//! path without naming [`crate::warm_start::pairing::TAKING`] at all. `preBuild` is the only phase read, so a consumer that
...
//! [`crate::warm_start::sweep`], which runs the real script over a real directory and asserts `try_exists` on
...
//! rules and [`crate::warm_start::sweep`]'s standalone run alike. What IS held is that the inline sits in the
//! string a `${..}` expands rather than anywhere in the file - [`crate::warm_start::pairing::module_sweeps`] carries the
