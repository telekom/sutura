...
| `sutura-exec-postgres` | `Warehouse` over Postgres on a static credential (the shared identity, no impersonation). Pure-Rust driver, so links nothing. `NoTls`, localhost tier only. A dev-dependency. Its corpus and differential cells are fail-closed: they run inside `checks.nextest`, which provisions nixpkgs' `postgresql_18` on a unix socket via `nix/postgres-tier.nix` - the same script is the one place Postgres is provisioned |
