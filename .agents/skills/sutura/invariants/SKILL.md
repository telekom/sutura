...
| A crate that is not a caller of the driving port is outside the scan, and so is `#[cfg(test)]` inside one | The rule is about what a published binary does, and test code is not in one. The scan reads a PATH rooted at `sutura_app`, so `use sutura_app as app;` then `app::answer(..)` is invisible - `use sutura_app::*` is not, because `clippy::wildcard_imports` is on and the gate runs with `-D warnings` |
