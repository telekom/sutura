...
`SET LOCAL statement_timeout` after acquiring its execution lock. BigQuery sends what remains as
`timeoutMs`/`jobTimeoutMs`; its live stopped-job reply is not measured, and the socket allowance can
cross the transport's reply margin. The concurrency bound is still necessary: it caps how much work
can accumulate where cancellation is delayed, cooperative or absent.
...
statement stop; for BigQuery it is after the service or socket answers. The service is asked to stop
the job at the deadline, but that live reply is unmeasured. The backlog is therefore bounded even
when a timed-out caller cannot promptly stop the underlying work.
...
reaches the data system for Postgres and the in-process engine, not for BigQuery.** The value is
carried to the BigQuery transport and the ADBC driver is handed no bound from it, so a job that
...
system - `docs/adr/0017`'s nineteenth amendment says why they are still required. What this
deployment DOES bound is `governance.per_replica_spend_ceiling`, charged from the dry run's
ESTIMATE, per subject, inside one replica's own window.
