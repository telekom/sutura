...
fn main() {
    ...
    // ---- LOCAL CHANGE (sutura). Prefer a prebuilt static archive when one is offered. ----
    //
    // WHY. This crate is a path dependency inside our workspace, so crane's shared
    // dependency build does not shield it the way it shields a registry crate: without this,
    // every edit to our own Rust recompiles mimalloc's C, once per target. `flake.nix`
    // compiles the same translation unit with the same flags in its own hash-addressed
    // derivation, so the C is built once per (target, opt-level) and is then cached in the
    // Nix store and in CI. Our source changes cannot invalidate it.
    //
    // The absent case is upstream's behaviour untouched: with the variable unset this falls
    // through to the `cc::Build` path below, so a bare `cargo build` on a machine without
    // nix still works. `SUTURA_` prefixed so it cannot be confused with a cc-rs or crane
    // variable, and `rerun-if-env-changed` is emitted unconditionally so that appearing or
    // disappearing is itself a reason to re-run.
    println!("cargo:rerun-if-env-changed=SUTURA_MIMALLOC_LIB_DIR");
    if let Ok(lib_dir) = env::var("SUTURA_MIMALLOC_LIB_DIR") {
