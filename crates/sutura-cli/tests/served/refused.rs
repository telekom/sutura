//! The four startup refusals `github.com/telekom/sutura#302` names, split out of `served.rs` by
//! `cargo xtask max-lines`'s 1000-line cap - a pure relocation, the same shape
//! `served/agent.rs`/`served/datahub.rs` already use. No test renamed, no behaviour changed.
//!
...
mod tests {
    ...
    /// standard error behind `commands::report`'s own `sutura: ` prefix - the same one every other
    /// command's failure gets, since `github.com/telekom/sutura#685` step 2 folded this command's
    /// own `sutura-serve: ` prefix away with the binary that used to print it.
