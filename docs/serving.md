...
directory of its own - the settings default (`catalog.dir: catalog`) is what resolves here because
the current directory is `examples/single-player`. An agent client launches that process and
speaks the protocol on its pipes - the same two tools this
...
| `catalogs[].kind`                               | `markdown`                           | Which adapter opens this catalog: `markdown`, `datahub`, `okf`, `openmetadata` or `rdbms`. `markdown` and `okf` are openable in every build (neither adapter carries a native or outbound-TLS dependency); `datahub` needs `--features datahub` and is refused by name without it; `openmetadata` and `rdbms` are declarable words this build cannot open AT ALL yet - refused by name naming the follow-up issue, since neither adapter crate has a reader over anything but a recorded fixture (`github.com/telekom/sutura#970`). **A deployment declares catalogs of ONE kind** - two entries of different kinds together are refused, not merged; see `docs/adr/0016` |
