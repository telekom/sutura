...
| `FederatedAnswerNotWellFormed`      | `409`  | **Added by D19 + A4.** A non-finite ratio or an ambiguous join across two legs is the SAME plan against the SAME rows failing again, which is what `409` says and what the `503` this used to leave as - "worth retrying" - does not. The same grouping as the rows above: answerable in principle, and this deployment cannot certify this particular combine. Carries no cell: `FederatedAnswerRefusal` is a bare discriminant, never the join key or the float value that produced it. **Limit stated where it belongs:** the mono (single-source) path has no equivalent refusal yet - the same non-finite condition there still leaves as a retryable `503` through `SurfaceFailure::Warehouse` / `Failure::Unavailable`, out of scope for this row and named in `.agents/skills/sutura/invariants/SKILL.md`                                                                                                                                                                                                                                                                                                                                                                                                |
| `PlanTablesShareAnIdentifier`       | `409`  | **Added by [0019](0019-a-table-outside-the-connections-dataset.md) Decision 5, corrected by D7.** Two tables the plan would read end their paths in one name, so a column qualified by it names neither. The same grouping as the rows above and for the same reason: answerable in principle, and this deployment cannot express it as one statement. Unlike those rows there usually *is* another question - a dimension needing no join - so the sentence says so. **D7: the sentence names neither the identifier nor either path.** The identifier is the thing that must not reach the wire, not the thing a person acts on: it is a table name reaching an agent's own context exactly as a schema name in a generated prompt would, and this refusal is a tool output an agent reads. Both transports hold it out of their own sentence independently - re-interpolating `{table}` back into either reddens a dedicated redaction test                                                                                                                                                                                                                                                                   |
| `ResultTooLarge`                    | `413`  | The answer did not fit the certifiable cap. Narrowing helps and repeating does not. **Amended after this record was accepted:** the variant now carries a `ResultBound` and covers a second bound - a data system that will not hand the result back in one piece - at the same status and the same code, because *too much data* is one answer and one remedy to a caller. Only the sentence differs, from a nested exhaustive match, and the second bound's carries no number: the bound belongs to the data system and is not reported to us. The widening exists because that case used to arrive as `503`, which this record calls *the one refusal where retrying is reasonable* - and a retry returned the same reply. **Amended again, `telekom/sutura#639`:** a third arm, `Encoded`, covers a result inside every other bound that is still wider than this deployment will spend encoding into a response - a caller's own grouped column can be arbitrary text a data system returns, which no type here bounds the length of. Unlike the volume arm this one carries a number, because it is this deployment's own ceiling - a compile-time constant, not a setting - and not one it was never told |
...
| ~~`LegsDecideIdentityDifferently`~~ | -      | **Added after this record was accepted, then DELETED by `0040-a-cross-posture-federated-answer-is-disclosed-per-leg.md`.** It refused an answer whose two legs would run under different identity postures. BigQuery is the only adapter that can carry a per-subject credential, so that refused every BigQuery federation with a shared-posture adapter rather than an edge case; a cross-posture answer is a `200` naming which leg came from which. The row is kept struck rather than removed because this table is cited by ordinal and by name                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- **The example session under `examples/single-player/README.md` showed captured `200 OK`
  ...
  `PlanTablesShareAnIdentifier`, `ResourcesExhausted`, `CredentialUnavailable` and `SourceRefused` -
  each of which needs a fixture that pair cannot produce. So the exhaustive claim is the unit
  table's, not the transport harness's. **`LegsDecideIdentityDifferently` was on that list and is
  not a variant any more**, per
  `0040-a-cross-posture-federated-answer-is-disclosed-per-leg.md`.
