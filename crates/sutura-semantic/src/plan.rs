...
mod tests {
    ...
    use sutura_domain::catalog::{Anchor, Description, Metric, Model};
    ...
    /// The direct, compiler-level refusal: a measure a combiner cannot re-derive - a distinct count -
    /// splitting across two sources is refused as `MeasureDoesNotFederate` by the splitter itself,
    /// before any leg is planned.
    #[test]
    fn a_distinct_count_is_refused_before_planning() {
        let source = SourceName::parse("facts").expect("a test source");
        let table = TableName::parse("orders").expect("a test table");
        let model = Model::new(
            ModelName::parse("orders").expect("a model"),
            source,
            table,
            BTreeSet::from([column("customer_key")]),
            Description::default(),
        );
        let distinct = Measure::Simple(Term::Aggregate(AggregatedColumn::new(
            Aggregate::CountDistinct,
            column("customer_key"),
        )));
        let metric = Metric::new(
            MetricName::parse("unique_customers").expect("a metric"),
            model.name().clone(),
            distinct,
            Vec::new(),
            column("order_date"),
        ...
        let resolution = Resolution {
            ...
            .expect("a test range"),
            keys: Vec::new(),
            filters: Vec::<ResolvedFilter<'_>>::new(),
        };

        match super::federated_plan(&resolution) {
            Err(RefusalReason::MeasureDoesNotFederate {
                metric: name,
                aggregate: Aggregate::CountDistinct,
            }) => {
                assert_eq!(name, MetricName::parse("unique_customers").expect("a metric"));
            }
            other => panic!("a distinct count cannot cross sources: got {other:?}"),
        }
    }
}
