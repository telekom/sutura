...
1. ~~**Exclude ADR pages from the workflows.**~~ **DONE, in three parts, and the parts are recorded
   ...
   **Reads code.** A diff of `docs/*.md` and `mkdocs.yml` cannot change a byte any of these reads, so
   ...
   | `check-catalog-opened-once` | the non-test Rust under `crates/sutura-catalog-local/src/`, `crates/sutura-catalog-okf/src/`, `crates/sutura-catalog-datacontract/src/` and `crates/sutura-bounded-read/src/`, for a path-based `std::fs` read outside the registered `read_dir` in the shared walk |
   ...
   **Reads prose.** Each of these reads at least one file such a diff CAN change, so skipping one
   ...
   | --- | --- |
   | `check-guidance` | Citations replaced by the text-only check. **Stale phrases, a version beside a pinned name, contradicted claims and counts are deferred to the `main` push** |
