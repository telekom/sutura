...
- **DataHub tier parallelism holds for the CONTAINERS and not for DISCOVERY** - project name from a
  ...
  (`nix/tier-endpoints.nix` merges per service), and the consequence is gone for both nix tiers:
  the dropped entry reads as *unclaimed* and the next `start` republishes it over the server that
  is already running - `just test` for postgres, `just keycloak-tier start` for keycloak, held by
  `checks.postgres-tier` and `checks.keycloak-tier`. What nothing gates is the wholesale write
  itself, so the entry still goes and a reader between the two commands still gets nothing.
