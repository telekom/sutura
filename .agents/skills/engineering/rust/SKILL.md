...
| an expected outcome returned as `Err` | a variant of the result. A governance refusal is `ToolOutcome::Refusal { reason }`, never an error | *review* for the CHOICE of `Ok` over `Err`. `check-refusal-coverage` fails a `RefusalReason` or `NotFitToServe` variant no test NAMES and no snapshot records, unless its separate allow file excuses it with a date and reason. **A name is not a provocation or venue execution**; snapshot words are unqualified. `xtask/src/refusals.rs` holds that narrow check, not the choice of result type |
...
| `cargo xtask check-refusal-coverage` | a `RefusalReason` or `NotFitToServe` variant no test names and no snapshot records, unless separately excused - a file naming EVERY variant of an enum is a census and counts for none of that enum |
