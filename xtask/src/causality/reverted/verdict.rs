...
mod tests {
    ...
    fn a_red_run_contradicts_this_rule_per_file_and_an_ordinary_pass_prints_nothing() {
        ...
        assert!(contradiction(&red(), &Reverted::Behaviour).is_empty());
    ...
    fn the_falsifier_fires_on_both_red_arms_and_on_nothing_else() {
        ...
        assert!(contradiction(&BaseOutcome::Green, &excused).is_empty());
        assert!(contradiction(&BaseOutcome::DidNotCompile, &excused).is_empty());
        assert!(contradiction(&red(), &Reverted::Behaviour).is_empty());
        assert!(contradiction(&outside, &Reverted::Behaviour).is_empty());
    }
}
