...
| A second execution leg | One answer has one asker, and no leg runs as a third identity: each runs as the asker **or** under that source's acknowledged shared identity, and **the answer records which, per leg, even where the two disagree** (`docs/adr/0040`). *"Every leg runs as the same subject"* was the wording here and was overstated - a source serving everyone as one identity does not run as the asker, and making the labels agree would not have made the identities agree. *"Every leg decides identity the SAME way, or the question is refused"* replaced it and is also gone: BigQuery is the only impersonating adapter, so that refused every heterogeneous federation | Recording and credentials are built and the refusal is deleted; see `../identity/SKILL.md`. A PUBLISHED build can record two different postures on one answer: the release links `bigquery`, which executes its leg as the asker, beside leg-executing adapters that are `NoPlaceForASubject` (`postgres`, and `oracle` where linked) - which answer, per leg, the shared identity the operator acknowledged for that source. **The record is a disclosure, not a control** - it leaves in the same body as the rows; the control is the boot acknowledgement on each source's own entry. **Not the promise:** no test asserts two subjects get different ROWS - that needs a live dataset with row-level security and two real grants |
...
operating-system identity. **Which is also why no PUBLISHED build can produce a mixed-posture answer:**
no published artefact links the only `PerSubjectCredential` adapter, so a release cannot reach a
source of each kind. **The refusal that used to compare the two postures is DELETED**
(`docs/adr/0040`): a `files` leg beside a `bigquery` impersonating leg is now ANSWERED, with one
`executed_as` entry per source naming that leg's own posture, because BigQuery is the only
impersonating adapter and refusing the mix refused every heterogeneous federation. Two impersonating
`bigquery` legs federate with a per-subject credential at each
