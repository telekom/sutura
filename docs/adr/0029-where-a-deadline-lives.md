...
Status: **accepted; the first slice lands the record, the port signature and the refusal, and the
second (the engine honours it) has landed too - BigQuery and Postgres still accept the deadline and
ignore it**, in slices that follow behind `telekom/sutura#160`. Four accepted
...
| `sutura-exec-postgres` (dev-dependency) | `SET statement_timeout = <remaining ms>` sent before each statement, replacing the one set at connect from a tuning value; `SQLSTATE 57014 query_canceled` is the predicate                                                                                                                                                                                                                                              | The server aborts the statement itself                                                                                                                              | The round trip that sets it is bounded by nothing but the socket; `57014` is also what an operator's manual cancel produces, and the predicate cannot tell the two apart                                                              |
...
| The engine stops at the deadline                            | `sutura-exec-datafusion`'s `execute` wraps `rows` in `tokio::time::timeout` on a runtime built with `enable_time()` - `execute_tests.rs::deadline_tests::a_question_that_exceeds_its_budget_is_stopped_at_the_data_system` (and its `with_worker_threads` twin) register a `StreamingTable` that never runs out on its own, open a 200ms budget, and assert the call returns inside the budget plus a 300ms tolerance and long before the source's own 2s end, the predicate answers `true`, and the source's own `Drop` fired - proving cancellation and not merely giving up on the wait; `a_spent_deadline_never_lets_the_plan_touch_the_source` asserts a budget already spent stops the plan before the source is ever asked, by counting its own executions | BigQuery and Postgres still accept the deadline and ignore it; a non-yielding operator, planning (the analyzer and optimizer), and one started blocking file read stay unreached even on the engine. **The stop is at the next cooperative yield, not at the instant** - each runnable partition task spends a 128-batch budget before the timer is heard, so the overrun scales with `target_partitions` x batch cost: measured 825ms against a 200ms budget for a 327M-row in-memory aggregate with 12 partitions on the current-thread runtime (`SessionConfig::new()`'s default), 290ms with 2 workers |
