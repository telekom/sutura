...
| `catalogs[].endpoint`                           | absent, **required for `datahub`**   | `datahub` only. `scheme://host[:port]`, no trailing slash. `https://` to any host, **verified against the binary's compiled-in roots - no deployment CA yet, `security.outbound` is the future seam (#742 lineage)**; `http://` only to an IP loopback literal - anything else is a startup refusal naming the endpoint                                                                                                                            |
