...
| `catalogs[].kind`                               | `markdown`                           | Which adapter opens this catalog. `markdown`, `datahub`, `okf`, `openmetadata` or `rdbms`. `datahub`/`okf` open behind their default-off `--features datahub`/`--features okf`; a build without the feature refuses the kind by name, naming it. `openmetadata`/`rdbms` are declarable and refused by name on every build (no reader over a real deployment yet - see the follow-up to `github.com/telekom/sutura#152` and `#972`). **A deployment declares catalogs of ONE kind** - a `markdown` and a `datahub` entry together are refused, not merged; see `docs/adr/0016`                                                                                            |
