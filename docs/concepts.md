...
!!! warning "Two kinds of claim on this page, marked at each one"
    ...
    **The identity claims are the ones to read carefully, and they are no longer all design targets.**
    ...
    system that evaluates the asking subject: no published adapter can carry a per-subject
...
the same, because no published adapter can carry a per-subject credential.
...
that ships mints from configuration and performs no token exchange. The default-off `bigquery`
feature builds one that does carry one, federated against a declared pool and unproven against a
live one. So what a credential says today is *the deployment's own identity for this source,
...
verifies a caller's own token. What no PUBLISHED build has is an adapter that can carry a
per-subject credential, so per-caller ACCESS is still absent there - and a deployment behind only the bearer token has
