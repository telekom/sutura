...
## Amendment: the licence report ships as a signed, committed document

**This amendment closes the half the roadmap bullet still names: *"signed licence reports"*. The
licence report was a CI artifact** - produced by `licence-review.yml`, kept with `actions/upload-artifact`,
expiring after 90 days, signed by nothing - and there was no attribution document at all,
`NOTICE` naming only `VENDOR.md`. A consumer who had a signed binary and a signed SBOM could not
obtain or verify a signed statement of what they were permitted to do with either.

The decision this amendment makes is two things and no more - **what the attribution document is**,
and **where it lives** - and the plan *What is claimed, and what is not* above is updated by it: the
sentence *"There is no attribution document, and `NOTICE` still says nothing about third-party
crates"* is superseded on its first half.

### What the attribution document is

**`THIRD_PARTY_NOTICES` at the repository root: every crate the workspace's `Cargo.lock` resolves,
with its resolved version, under a header that states what the list is and is not. It is generated
and it is an inventory, not a verdict.**

The per-identifier policy stays where it always was - `deny.toml`'s allowlist and the scheduled ORT
review decide whether a licence is acceptable - and `NOTICE` stays the first-party statement. What
this document adds is the third-party half: a named crate, at a named version, is traceable to a
licence obligation, and an auditor can ask "is this crate in the release, and at which version?" of
a single artefact rather than of a CI artifact that has expired.

### Where it lives, and why committed

**Committed, and regenerated by a gate - `cargo xtask check-attribution` - with the regeneration
checked rather than trusted.** This is the *Canonical Sources* rule applied to an artefact this
repository owns: one owner, generated, and a gate that fails when the generated file and its source
diverge. `release.yml` copies it into `dist/` on every tag, so it is signed - Sigstore bundle and
SLSA provenance - and attached to the release like every other asset, which is what makes it the
roadmap's *signed licence report*.

**Why `Cargo.lock` here when the SBOM edits it out, and this is the half of the relationship that is
easy to get wrong.** The SBOM is an *inventory*: it must name what a binary actually links, so the
`cargo auditable` section inside the binary is its source and a `Cargo.lock` document beside it
would drift and would overstate. Attribution is the opposite direction. Failing to attribute a crate
a release could carry is the failure an auditor disproves, and naming more than any one binary links
is safe where naming less is not - so the whole resolved set is the honest, conservative answer, and
the lock file is its source. One owner per artefact: the SBOM is the auditable binary's, this
document is the lock file's. The two are different claims and the difference is the safe direction
of each.

**Why not the ORT report as the release's document.** ORT's curated report is the weekly review - a
JVM render over the whole closure that needs network and minutes, is consumed by a person, and
cannot be regenerated inside a gate or the nix sandbox. Its output is evidence for review, not the
release's obtainable statement; that statement is now the committed, signed document above, which is
cheap, hermetic and always current because it is a function of the lock file.

### What it costs

**Nothing in a binary and nothing in the SBOM.** The document is generated by an `xtask` gate that
reads `Cargo.lock` as text and diffs - no compiler, no network, no registry cache, one new `Hygiene`
gate in a sweep that already builds nothing. **No licence identifiers in the document**, which is
the honest cost of the shape: `Cargo.lock` carries no licences, and resolving them needs the
registry cache the sandbox does not have, so identifiers live in `deny.toml` and the ORT report and
the document says so in its own header rather than pretending it carried a verdict it does not.

### What is now claimed, and what still is not

Claimed, and checkable:

- The licence report for a release is obtainable for that **specific** release and verifiable
  offline: `cosign verify-blob --bundle THIRD_PARTY_NOTICES.sigstore.json ...` from the release
  page, exactly like the other assets, documented in `docs/verifying-a-release.md`.
- The attribution document names every crate the shipped binaries link: it gates against the whole
  resolved set, which is a superset of any binary's linked set, so `check-attribution` failing
  means Cargo.lock and the document have drifted.

Not claimed, and each is a real edge:

- **Not a statement about the crates.** The document lists what the lock resolved and at which
  version; an advisory against one is `cargo-deny`'s business and a licence question is ORT's and
  `deny.toml`'s.
- **Not per-binary.** One document covers the release, by design - attribution errs toward the
  whole set, and the SBOM beside it is where per-binary precision lives.
- **Not legal advice.** A list of names and versions is the raw material of attribution, not the
  notice text of every licence; this repository's position on which notices must travel remains the
  review's, populated from the list this document pins.
