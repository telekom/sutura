...
**Twenty-six steps, sixteen done.** Every number is counted off the table below rather than
...
| 11    | `feat/conformance-packs`              | 10 - **done**                                                                                                                            | **#116**, and partly built: `crates/sutura-conformance` holds the pack bodies, and `cargo xtask check-conformance-bindings` holds every registered data system bound to the execute packs, in both directions, with an empty exemption list. What is left is what [0012](adr/0012-conformance-packs-for-inputs-and-adapters.md)'s status now names: a case is a constructor rather than a file, and the cases that record names by name are unwritten                                                                                                                                                                                                                                                                                                                                    |
...
| 15    | `feat/bigquery-adapter`               | 8 - **done**; the fixture decision is **made**, `adr/0017`; the dependency decision is **made**, `adr/0018`                              | **MOSTLY DONE, and further than the previous text said.** The fourth dialect and its goldens, the adapter, the source declaration, the registry entry, and the former HTTP wire (`jobs.query` over `ureq`) plus a credential port - and since then **the CORPUS leg, green in CI**: 22 statements accepted, 21 answers agreeing with the engine exactly, 9 refusals agreeing, 6 anchors reproduced, per `adr/0017`'s third and fourth amendments. `sutura-serve` **opens** `kind: bigquery` now, behind a default-off feature. Since #929, published artefacts link the ADBC adapter and its driver (`nix/shipped.nix`); the earlier HTTP wire and default-feature release limit no longer apply. The current limit is venue evidence: no served cross-posture run is observed           |
| 16    | `feat/bigquery-impersonation`         | 12, 15, and the ID-token verification                                                                                                    | **#87**, and three of its five parts are built: the adapter declares `PerSubjectCredential`, the ADBC transport hands the asker's own assertion to Google's token service to federate, and `sources.<alias>.workload_identity` is declarable. The exchanging broker that used to be named here is deleted (`docs/adr/0018`, eighth amendment). What is left is the proof - #147 for the venue, #123 for the two-principal cell                                                                                                                                                                                                                                                                                                                                                           |
| 17    | `feat/postgres-adapter`               | 8 - **done**, 14, and the artifact question                                                                                              | **DONE for the served source.** The adapter is held to the corpus, the differential and the anchors against a real `postgresql_18` in the nix tier; `SourceKind::Postgres` is declarable with every key the connection needs, both composition roots open it behind a default-off `postgres` feature, and the posture cross-check is against this adapter's own constant - **#124**. The serving guide carries the operator declaration and least-authority grants, and the served e2e loads the single-player data into the tier and pins its certified answer over HTTP. TLS to it is row 19's and landed with it (**#125**), because #124 alone would ship a source over a connection on which nothing verifies a certificate                                                         |
...
| 19    | `feat/source-mtls`                    | 8 - **done**, 17                                                                                                                         | **#125, and the Postgres half has landed** (one pull request with #124). `sutura_config::sources::transport` owns the three-state `SourceTransport` with `TrustAnchors`/`ClientIdentity`; a TLS mode with no anchors refuses at load, a partial client identity names the missing half, and a source with no transport security and a non-loopback host refuses naming `transport_mode`. `sutura-exec-postgres` connects over `tokio-postgres-rustls` from the declared bundle or explicitly selected host store with the handshake mandatory. The tier proves verified TLS and a certificate-authenticated role that rejects a client identity omission. Not reached: rotation, and the HTTP adapters - the BigQuery wire still verifies against whatever `ureq` is configured to trust |
...
| 21    | ~~`feat/demo-tasks`~~                 | 14 - **done**, and one example to demo                                                                                                   | **DONE** - **#595**. `just demo` brings the shipped server up over `examples/single-player` beside Open WebUI v0.11.3, registered through its native OpenAPI connection so there is no plugin to maintain, with the model configurable - hosted with a key, or local - and the browser URL read out of the tier's discovery file rather than written down. `docs/demo.md` is the walkthrough, and it states both limits: the chat client is ungoverned, and a single-user demo proves neither caller identity nor source impersonation                                                                                                                                                                                                                                                   |
...
| 24    | `feat/metrics-endpoint`               | 6 - **done** for the memory series, nothing for the rest                                                                                 | **#132 is closed and the endpoint shipped**: the registry, the authenticated gate, the series in [0015](adr/0015-an-authenticated-metrics-endpoint.md)'s table, and the query-completion accounting. What is left is `sutura_build_info` and the engine-pool series that record specifies and deliberately holds back, so this row is not struck; 0015's amendment names each deviation                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### Two orderings in this set were security constraints, not preferences
...
- **The exporters must not land before a principal's printing is shaped, and they did not.** A served
  deployment exported nothing, which is why a refusal and a fault looked the same to an operator, and
  the step that fixed it added exporters. **Adding exporters is the moment principals start being
  exported**, so the masking work had to go first or in the same pull request - it was #388 against
  #132. It went in the same work: both are closed as COMPLETED, and the parse-boundary half of #388
  landed first, a principal being masked where it is parsed rather than where it is rendered. What
  the endpoint does and does not export is decided in `docs/adr/0015`, not here.
- **A declarable Postgres source and its transport security are ONE pull request, and this landed as
  one change.** `SourceKind` had no `postgres` - so no deployment could declare one (#124) - and
  `sutura-exec-postgres` connected with `NoTls` unconditionally (#125), which
  `crates/sutura-exec-postgres/src/lib.rs` said on the module itself. Either alone would have shipped
  a deployment-declarable data source over a connection on which nothing verifies a certificate. The
  same change makes the source declarable, gives it a per-source transport declaration with the
  refusals at load, and connects it over TLS with the declared anchors - so there is no release in
...
one adapter can carry a per-subject credential and refusing the mix prevented BigQuery
from federating with a shared-posture adapter. What holds the ordering's original worry is the BOOT refusal
