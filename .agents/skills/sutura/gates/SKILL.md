...
## The fuzz harness, and why none of it is a gate

`fuzz/` holds four libFuzzer targets over the parsers that read input this deployment does not
write. Nothing there gates a merge, and the split is worth knowing before believing either half:

| Thing | When it runs | What a green one means |
| --- | --- | --- |
| `cargo xtask check-fuzz` | every pull request, in the hygiene sweep | every target file is declared as a `[[bin]]`, names `libfuzzer_sys`, has a tracked seed, and is in the workflow's matrix. It reads no code and runs no fuzzer. |
| `just fuzz-smoke` | the fuzz workflow, on a change to `fuzz/` itself | every committed seed still passes its target's assertions. Deterministic, and the only half that could be a gate. |
| `just fuzz` | `.github/workflows/fuzz.yml` on a schedule | a budget was spent and nothing crashed *in that budget*. Not a proof of absence, and next week's run explores different bytes. |

**A fuzz target that runs and finds nothing is the failure mode to look for, not the success
case, and this repository has already produced one.** `catalog_document` writes its input to a file
and loads it through the real `SemanticCatalog` port, which reads the document with
`read_to_string` - so while the target wrote RAW bytes, every mutation that introduced a multibyte
character had to leave the whole rest of the file valid UTF-8, and one that did not was rejected
before any parser ran and returned no coverage to steer by. Measured against a planted
char-count-as-byte-offset defect in the frontmatter splitter: **27305 executions over four minutes
found nothing**; converting the input with `String::from_utf8_lossy` first found the same defect at
execution **717**. The gates above cannot see that class of blindness - `check-fuzz` reads the
harness's shape, not what it reaches - so it is a review question, and each target's module header
is where the reach and its limits are written down.

**`panic = "abort"` does not blind libFuzzer, measured rather than assumed.** The fuzz crate is its
own workspace, so the root manifest's profiles do not reach it and its own `[profile.release]`
declares the shipped strategy deliberately. The planted panic went through
`std::process::abort`, libFuzzer's `SIGABRT` handler reported `deadly signal`, and the artifact was
written under `fuzz/artifacts/<target>/` with the base64 of the input - so an abort is still
attributed to the input that caused it. `libfuzzer-sys` installs a panic hook that aborts anyway,
so the strategy changes what is compiled rather than whether a finding is reportable.

**What is NOT fuzzed, and it is not a short list.** Everything `TokenValidator::verify` does after
the signature check - the claims deserialization, the `typ` class check, the actor-nesting bound -
is behind a signature generated bytes cannot forge, and reaching it needs a harness holding a
*fixed* signing key, which in a public repository is a committed private key. DataFusion, DuckDB and
the dialect renderers are not fuzzed at all. `polyglot-sql`'s empty-token-list abort is
**lint-held rather than input-held** (`clippy.toml` bans the call) and no target can reach it until
the authored-SQL hatch exists. And nothing here is property-tested: `proptest` is absent from the
tree, so the semantic invariants - normalisation idempotence, serde round-trip - are stated in doc
comments and checked by examples only.

**`cargo deny` does not read `fuzz/Cargo.lock`.** It reads the root lock, and the fuzz crate is a
separate workspace with a lock of its own - so its two third-party direct dependencies
(`libfuzzer-sys`, `serde_json`) are advisory-checked by nothing.
