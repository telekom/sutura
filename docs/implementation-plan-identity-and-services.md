...
1. ~~**Exclude ADR pages from the workflows.**~~ **DONE, in three parts, and the parts are recorded
   ...
   **Reads code.** A diff of `docs/*.md` and `mkdocs.yml` cannot change a byte any of these reads, so
   ...
   | `check-catalog-opened-once` | the non-test Rust under `crates/sutura-catalog-local/src/`, `crates/sutura-catalog-okf/src/` and `crates/sutura-catalog-datacontract/src/`, for a path-based `std::fs` read outside the registered `read_dir` walk |
